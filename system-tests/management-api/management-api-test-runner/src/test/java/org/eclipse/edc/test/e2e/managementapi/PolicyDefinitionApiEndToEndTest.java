...
public class PolicyDefinitionApiEndToEndTest
  ...
  void shouldStorePolicyDefinition() {
    var requestBody =
        createObjectBuilder()
            .add(CONTEXT,
                 createObjectBuilder().add("edc", EDC_NAMESPACE).build())
    ...
    assertThat(store().findById(id))
        .isNotNull()
        .extracting(PolicyDefinition::getPolicy)
        .isNotNull()
        .extracting(Policy::getPermissions)
        .asList()
        .hasSize(1);
    ...
    baseRequest()
        ...
        .log()
        .all()
        .body("policy.'odrl:permission'.'odrl:constraint'.'odrl:operator'.@id",
              is("odrl:eq"));
  }
  ...
  void shouldStorePolicyDefinitionWithPrivateProperties() {
    var requestBody =
        createObjectBuilder()
            ...
            .add(TYPE, "PolicyDefinition")
            .add("policy", sampleOdrlPolicy())
            .add("privateProperties",
                 createObjectBuilder().add("newKey", "newValue").build())
            .build();
  ...
  void shouldUpdateWithProperties() {
    ...
    var updatedBody =
        createObjectBuilder()
            ...
            .add("policy", sampleOdrlPolicy())
            .add("privateProperties",
                 createObjectBuilder().add("newKey", "updatedValue").build())
            .build();
    ...
    assertThat(result).extracting(Entity::getCreatedAt).isNotEqualTo(createdAt);
  ...
  void shouldDelete() {
    ...
    baseRequest().delete("/v2/policydefinitions/" + id).then().statusCode(204);

    baseRequest().get("/v2/policydefinitions/" + id).then().statusCode(404);
  }
