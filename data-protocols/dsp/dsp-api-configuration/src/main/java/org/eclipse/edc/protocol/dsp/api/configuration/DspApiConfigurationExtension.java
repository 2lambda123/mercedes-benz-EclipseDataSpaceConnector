...
/**
 * Provides the configuration for the Dataspace Protocol API context. Creates
 * the API context using {@link #DEFAULT_PROTOCOL_PORT} and {@link
 * #DEFAULT_PROTOCOL_API_PATH}, if no respective settings are provided.
 * Configures the API context to allow Jakarta JSON-API types as endpoint
 * parameters.
 */
...
public class DspApiConfigurationExtension implements ServiceExtension {
  ...
  public static final String NAME =
      "Dataspace Protocol API Configuration Extension";

  public static final String CONTEXT_ALIAS = "protocol";

  public static final String DEFAULT_DSP_CALLBACK_ADDRESS =
      "http://localhost:8282/api/v1/dsp";
  ...
  public static final WebServiceSettings SETTINGS =
      WebServiceSettings.Builder.newInstance()
  ...
  @Inject private WebServiceConfigurer configurator;
  ...
  public void initialize(ServiceExtensionContext context) {
    ...
    var dspWebhookAddress =
        context.getSetting(DSP_CALLBACK_ADDRESS, DEFAULT_DSP_CALLBACK_ADDRESS);
    context.registerService(
        DspApiConfiguration.class,
        new DspApiConfiguration(config.getContextAlias(), dspWebhookAddress));
    ...
    webService.registerResource(config.getContextAlias(),
                                new ObjectMapperProvider(jsonLdMapper));
    webService.registerResource(
        config.getContextAlias(),
        new JerseyJsonLdInterceptor(jsonLd, jsonLdMapper, DSP_SCOPE));
  ...
  private void registerTransformers() {
    ...
    transformerRegistry.register(
        new JsonObjectFromCatalogTransformer(jsonBuilderFactory, mapper));
    transformerRegistry.register(
        new JsonObjectFromDatasetTransformer(jsonBuilderFactory, mapper));
    transformerRegistry.register(
        new JsonObjectFromPolicyTransformer(jsonBuilderFactory));
    transformerRegistry.register(
        new JsonObjectFromDistributionTransformer(jsonBuilderFactory));
    transformerRegistry.register(
        new JsonObjectFromDataServiceTransformer(jsonBuilderFactory));
    transformerRegistry.register(
        new JsonObjectFromAssetTransformer(jsonBuilderFactory, mapper));
    transformerRegistry.register(
        new JsonObjectFromDataAddressTransformer(jsonBuilderFactory));
    transformerRegistry.register(
        new JsonObjectFromQuerySpecTransformer(jsonBuilderFactory));
    transformerRegistry.register(
        new JsonObjectFromCriterionTransformer(jsonBuilderFactory, mapper));
    ...
    // ODRL Transformers
    OdrlTransformersFactory.jsonObjectToOdrlTransformers().forEach(
        transformerRegistry::register);
