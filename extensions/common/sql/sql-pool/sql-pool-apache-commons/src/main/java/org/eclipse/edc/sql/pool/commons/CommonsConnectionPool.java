...
public final class CommonsConnectionPool
    implements ConnectionPool, AutoCloseable {
  ...
  public CommonsConnectionPool(
    ...
    Objects.requireNonNull(commonsConnectionPoolConfig,
                           "commonsConnectionPoolConfig");

    this.connectionObjectPool = new GenericObjectPool<>(
        new PooledConnectionObjectFactory(
            dataSource, commonsConnectionPoolConfig.getTestQuery(), monitor),
        getGenericObjectPoolConfig(commonsConnectionPoolConfig));
  }

  private static GenericObjectPoolConfig<Connection> getGenericObjectPoolConfig(
      CommonsConnectionPoolConfig commonsConnectionPoolConfig) {
    ...
    genericObjectPoolConfig.setMaxIdle(
        commonsConnectionPoolConfig.getMaxIdleConnections());
    genericObjectPoolConfig.setMaxTotal(
        commonsConnectionPoolConfig.getMaxTotalConnections());
    genericObjectPoolConfig.setMinIdle(
        commonsConnectionPoolConfig.getMinIdleConnections());

    genericObjectPoolConfig.setTestOnBorrow(
        commonsConnectionPoolConfig.getTestConnectionOnBorrow());
    genericObjectPoolConfig.setTestOnCreate(
        commonsConnectionPoolConfig.getTestConnectionOnCreate());
    genericObjectPoolConfig.setTestOnReturn(
        commonsConnectionPoolConfig.getTestConnectionOnReturn());
    genericObjectPoolConfig.setTestWhileIdle(
        commonsConnectionPoolConfig.getTestConnectionWhileIdle());
  ...
  public CommonsConnectionPoolConfig getPoolConfig() { return poolConfig; }
  ...
  private static class PooledConnectionObjectFactory
    ...
    PooledConnectionObjectFactory(@NotNull DataSource dataSource,
                                  @NotNull String testQuery, Monitor monitor) {
    ...
    private boolean isConnectionValid(Connection connection) {
      ...
      } catch (Exception e) { // any exception thrown indicates invalidity of
                              // the connection
        return false;
      }
    }
