...
public class LdpVerifier implements CredentialVerifier {
  ...
  private Collection<MethodResolver> methodResolvers =
      new ArrayList<>(List.of(new HttpMethodResolver()));
  ...
  /**
   * Verifies a VerifiableCredential (VC) or a VerifiablePresentation (VP),
   * represented as JSON-LD-string.
   *
   * @param rawInput        The raw JSON-LD string. This must be either a
   *     VerifiablePresentation or a VerifiableCredential.
   *                        Note that the JSON-LD will be expanded before
   * processing.
   * @param verifierContext The {@link VerifierContext} to which nested
  ...
  public Result<Void> verify(String rawInput, VerifierContext verifierContext) {
    ...
    return expansion.compose(expandedDocument -> {
      try {
        ...
        return failure(
            "Could not verify VP-LDP: message: %s, code: %s".formatted(
                e.getMessage(), e.getCode()));
      } catch (VerificationError e) {
        return failure("Could not verify VP-LDP: %s | message: %s".formatted(
  ...
  /**
   * Validates the credential issuer by comparing it with the provided
   * verification method.
   *
   * @param expanded           The expanded JSON-LD object representing the
   *     credential.
   * @param verificationMethod The verification method to compare with the
   *     issuer.
   * @return A {@link Result} indicating success or failure.
   */
  private Result<Void>
  validateCredentialIssuer(JsonObject expanded,
    ...
    try {
      ...
      if (!UriUtils.equalsIgnoreFragment(issuerUri.get(),
                                         verificationMethod.id())) {
        return failure(
            "Issuer and proof.verificationMethod mismatch: %s <> %s".formatted(
                issuerUri.get(), verificationMethod.id()));
      }
  ...
  /**
   * Extracts the first graph from a JSON-LD document, if it exists. When
   * multiple VCs are present in a VP, they are expanded to a {@code @graph}
   * object.
   *
   * @param document The JSON-LD document to extract the graph from.
   * @return The first graph from the JSON-LD document, or the original document
   *     if no graph exists.
   */
    ...
    if (document.getValueType() == JsonValue.ValueType.OBJECT) {
      if (document.asJsonObject().get(JsonLdKeywords.GRAPH) != null) {
        return document.asJsonObject()
            .getJsonArray(JsonLdKeywords.GRAPH)
            .get(0);
      }
    }
    return document;
  ...
  private Result<Void> verifyExpanded(JsonObject expanded,
    ...
    if (isCredential(expanded)) {
      ...
      for (var credential : JsonLdReader.getObjects(
               expanded, VcVocab.VERIFIABLE_CREDENTIALS.uri())) {

        if (JsonUtils.isNotObject(credential)) {
          return failure(
              "Presentation contained an invalid 'verifiableCredential' object!");
        }
      ...
      return credentials.stream()
          ...
          .reduce(Result::merge)
          .orElse(
              success()); // "no credentials" is still valid according to
  ...
  private Result<Void> verifyProofs(JsonObject expanded)
      throws VerificationError, DocumentError {
    ...
    for (var embeddedProof : proofs) {
      ...
      if (proofType == null || proofType.isEmpty()) {
        return failure("%s: %s, %s".formatted(ErrorType.Missing, VcVocab.PROOF,
                                              LdTerm.TYPE));
      }
      ...
      var signatureSuite =
          proofType.stream()
              ...
              .orElseThrow(
                  () -> new VerificationError(Code.UnsupportedCryptoSuite));

      if (signatureSuite.getSchema() == null) {
        return failure("The suite [" + signatureSuite.getId() +
                       "] does not provide proof schema.");
      }

      LdProperty<byte[]> proofValueProperty =
          signatureSuite.getSchema().tagged(VcTag.ProofValue.name());
      ...
      if (!proof.contains(proofValueProperty.term())) {
        return failure(
            "%s: %s".formatted(ErrorType.Missing, proofValueProperty.term()));
      }
      ...
      LdProperty<VerificationMethod> methodProperty =
          signatureSuite.getSchema().tagged(VcTag.VerificationMethod.name());

      if (methodProperty == null) {
        return failure(
            "The proof schema does not define a verification method.");
      }

      var verificationMethod =
          getMethod(methodProperty, proofObject, signatureSuite)
              .orElseThrow(()
                               -> new DocumentError(ErrorType.Missing,
                                                    methodProperty.term()));

      if (!(verificationMethod instanceof VerificationKey)) {
        return failure(
            "%s: %s".formatted(ErrorType.Unknown, methodProperty.term()));
      }
      ...
      // verify signature
      signature.verify(data, unsignedProof, (VerificationKey)verificationMethod,
                       proofValue);
    }
    // all good
    return success();
  ...
  private Optional<VerificationMethod>
  getMethod(LdProperty<VerificationMethod> property, JsonObject proofObject,
  ...
  private Optional<VerificationMethod>
  resolve(JsonObject method, SignatureSuite suite,
          LdProperty<VerificationMethod> property) throws DocumentError {
    try {
      var id = JsonLdReader.getId(method).orElseThrow(
          () -> new DocumentError(ErrorType.Missing, property.term()));
  ...
  private Optional<VerificationMethod>
  resolve(URI id, SignatureSuite suite, LdProperty<VerificationMethod> property)
    ...
    var resolver =
        methodResolvers.stream().filter(r -> r.isAccepted(id)).findFirst();
  ...
  public static class Builder {
    ...
    private Builder() { verifier = new LdpVerifier(); }

    public static Builder newInstance() { return new Builder(); }

    public Builder signatureSuite(SignatureSuite signatureSuiteProvider) {
      this.verifier.suiteProvider =
          new SignatureSuiteMapper().add(signatureSuiteProvider);
      return this;
    }
    ...
    public LdpVerifier build() {
      Objects.requireNonNull(this.verifier.jsonLd,
                             "Must have a JsonLD service!");
      Objects.requireNonNull(this.verifier.jsonLdMapper,
                             "Must have an ObjectMapper!");
      Objects.requireNonNull(this.verifier.suiteProvider,
                             "Must have a SignatureSuite!");
