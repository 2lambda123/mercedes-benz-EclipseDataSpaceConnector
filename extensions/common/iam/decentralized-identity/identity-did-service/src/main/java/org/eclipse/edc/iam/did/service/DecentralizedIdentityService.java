...
public class DecentralizedIdentityService implements IdentityService {
  ...
  public DecentralizedIdentityService(DidResolverRegistry resolverRegistry,
  ...
  obtainClientCredentials(TokenParameters parameters) {
    var jwt = JwtUtils.create(privateKey, issuer, issuer,
                              parameters.getAudience(), clock);
  ...
  verifyJwtToken(TokenRepresentation tokenRepresentation,
    ...
    try {
      ...
      var didResult =
          resolverRegistry.resolve(jwt.getJWTClaimsSet().getIssuer());
      if (didResult.failed()) {
        return Result.failure(
            "Unable to resolve DID: " +
            String.join(", ", didResult.getFailureMessages()));
      }
      ...
      var publicKeyWrapperResult = KeyConverter.toPublicKeyWrapper(
          publicKeyJwk, publicKey.get().getId());
      ...
      var verified = JwtUtils.verify(jwt, publicKeyWrapperResult.getContent(),
                                     context.getAudience());
      if (verified.failed()) {
        monitor.debug(()
                          -> "Failure in token verification: " +
                                 verified.getFailureDetail());
      ...
      var credentialsResult =
          credentialsVerifier.getVerifiedCredentials(didResult.getContent());
      if (credentialsResult.failed()) {
        monitor.debug(()
                          -> "Failed to retrieve verified credentials: " +
                                 credentialsResult.getFailureDetail());
        return Result.failure("Failed to get verifiable credentials: " +
                              credentialsResult.getFailureDetail());
      }
      ...
      var claimToken = ClaimToken.Builder.newInstance()
                           ...
                           .claim(DEFAULT_IDENTITY_CLAIM_KEY,
  ...
  firstVerificationMethod(DidDocument did) {
    return did.getVerificationMethod()
        .stream()
        .filter(
            vm
            -> DidConstants.ALLOWED_VERIFICATION_TYPES.contains(vm.getType()))
        .findFirst();
  }
}
