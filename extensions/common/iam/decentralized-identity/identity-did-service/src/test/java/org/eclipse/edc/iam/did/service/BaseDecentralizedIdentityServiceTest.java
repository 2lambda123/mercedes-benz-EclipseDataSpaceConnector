...
abstract class BaseDecentralizedIdentityServiceTest {
  ...
  private static final String DID_DOCUMENT =
      getResourceFileContentAsString("dids.json");

  private final CredentialsVerifier credentialsVerifierMock =
      mock(CredentialsVerifier.class);
  private final DidResolverRegistry didResolverRegistryMock =
      mock(DidResolverRegistry.class);
  ...
  void setUp() {
    identityService = new DecentralizedIdentityService(
        didResolverRegistryMock, credentialsVerifierMock, new ConsoleMonitor(),
        privateKeyWrapper(keyPair), DID_URL, Clock.systemUTC());
  }

  @Test
  void generateAndVerifyJwtToken_valid() {
    when(credentialsVerifierMock.getVerifiedCredentials(any()))
        .thenReturn(Result.success(Map.of("region", "eu")));
    when(didResolverRegistryMock.resolve(anyString()))
        .thenReturn(Result.success(createDidDocument(keyPair)));

    var result =
        identityService.obtainClientCredentials(defaultTokenParameters());
    assertThat(result.succeeded()).isTrue();

    var verificationResult = identityService.verifyJwtToken(
        result.getContent(), verificationContext("Bar"));
    ...
    assertThat(claimToken.getStringClaim(DEFAULT_IDENTITY_CLAIM_KEY))
        .isEqualTo(DID_URL);
  }
  ...
  void generateAndVerifyJwtToken_wrongPublicKey() {
    ...
    when(credentialsVerifierMock.getVerifiedCredentials(any()))
        .thenReturn(Result.success(Map.of("region", "eu")));
    when(didResolverRegistryMock.resolve(anyString()))
        .thenReturn(Result.success(createDidDocument(otherKeyPair)));
    ...
    assertThat(verificationResult.getFailureMessages())
        .contains("Token could not be verified!");
  }
  ...
  void generateAndVerifyJwtToken_wrongAudience() {
    ...
    var verificationResult = identityService.verifyJwtToken(
        result.getContent(), verificationContext("Bar2"));
    assertThat(verificationResult.failed()).isTrue();
  }
  ...
  @NotNull protected abstract JWK generateKeyPair();

  @NotNull protected abstract PrivateKeyWrapper privateKeyWrapper(JWK keyPair);
