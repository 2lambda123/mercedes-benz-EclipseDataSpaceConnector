...
class JsonObjectToPresentationQueryTransformerTest {
  ...
  private final JsonObjectToPresentationQueryTransformer transformer =
      new JsonObjectToPresentationQueryTransformer(mapper);
  ...
  void setUp() {
    jsonLd.registerCachedDocument(
        ...
        TestUtils.getFileFromResourceName("presentation_ex.json").toURI());
    jsonLd.registerCachedDocument(
        "https://w3id.org/tractusx-trust/v0.8",
        TestUtils.getFileFromResourceName("presentation_query.json").toURI());
  ...
  @Test
  void transform_withScopes_separatedByWhitespace()
  ...
  void transform_withPresentationDefinition() throws JsonProcessingException {
    var json = "" "
    {
      "@context": [
        ...
        "id" : "first simple example", "input_descriptors" : [ {
          "id" : "descriptor-id-1",
    ...
    assertThat(expansion.succeeded())
        .withFailMessage(expansion::getFailureDetail)
        .isTrue();
    ...
    assertThat(query.getPresentationDefinition().getInputDescriptors())
        .isNotEmpty()
