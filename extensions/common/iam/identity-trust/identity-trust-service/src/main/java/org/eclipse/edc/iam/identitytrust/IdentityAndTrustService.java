 ...
 *     <li>Establishes proof-of-original possession, by extracting the
 * PRESENTATION_ACCESS_TOKEN_CLAIM, and re-packaging it into a new SI token</li>
 *     <li>Performs a presentation request against a CredentialService</li>
 *     <li>Validates and verifies the VerifiablePresentation</li>
 * </ul>
 * This service is intended to be used together with the Identity And Trust
 * Protocols. Details about the scope string can be found <a
 * href="https://github.com/eclipse-tractusx/identity-trust/blob/main/specifications/M1/verifiable.presentation.protocol.md#31-access-scopes">here</a>
 */
public class IdentityAndTrustService implements IdentityService {
   ...
   */
  public IdentityAndTrustService(
      SecureTokenService secureTokenService, String myOwnDid,
      String participantId, PresentationVerifier presentationVerifier,
      CredentialServiceClient credentialServiceClient,
      JwtValidator jwtValidator, JwtVerifier jwtVerifier,
      TrustedIssuerRegistry trustedIssuerRegistry, Clock clock,
      CredentialServiceUrlResolver csUrlResolver,
      AudienceResolver audienceMapper) {
  ...
  obtainClientCredentials(TokenParameters parameters) {
    ...
    parameters.getAdditional().forEach(
        (k, v) -> claims.replace(k, v.toString()));

    claims.putAll(Map.of("iss", myOwnDid, "sub", myOwnDid, "aud",
                         parameters.getAudience(), "client_id", participantId));
  ...
  verifyJwtToken(TokenRepresentation tokenRepresentation,
    ...
    var claimTokenResult =
        jwtVerifier.verify(tokenRepresentation.getToken(), participantId)
            .compose(v
                     -> jwtValidator.validateToken(
                         tokenRepresentation,
                         participantId)) // audience must be set to my own
                                         // participant ID
    ...
    var accessToken =
        claimToken.getStringClaim(PRESENTATION_ACCESS_TOKEN_CLAIM);
    var issuer = claimToken.getStringClaim(ISSUER);
    var intendedAudience = claimToken.getStringClaim("client_id");

    /* TODO: DEMO the scopes should be extracted elsewhere. replace this
     * section!!############################*/
    var scopes = new ArrayList<String>();
    try {
      var scope = SignedJWT.parse(accessToken)
                      .getJWTClaimsSet()
    ...
    var siTokenClaims =
        Map.of(PRESENTATION_ACCESS_TOKEN_CLAIM, accessToken, ISSUED_AT,
               Instant.now().toString(), AUDIENCE, intendedAudience, ISSUER,
               myOwnDid, SUBJECT, myOwnDid, EXPIRATION_TIME,
    ...
    var vpResponse = credentialServiceUrlResolver.resolve(issuer).compose(
        url
        -> credentialServiceClient.requestPresentation(url, siTokenString,
                                                       scopes));
    ...
    var result =
        presentations.stream()
            .map(verifiablePresentation -> {
              var credentials =
                  verifiablePresentation.presentation().getCredentials();
              ...
              return presentationVerifier
                  ...
                  .compose(
                      u -> validateVerifiableCredentials(credentials, issuer));
            })
            .reduce(Result.success(), Result::merge);
    // todo: at this point we have established what the other participant's DID
    // is, and that it's authentic
    //  so we need to make sure that `iss == sub == DID`
    return result.compose(
        u
        -> extractClaimToken(
            presentations.stream()
                .map(p -> p.presentation().getCredentials().stream())
  ...
  @NotNull
  private Result<Void>
  validateVerifiableCredentials(List<VerifiableCredential> credentials,
                                String issuer) {
    // in addition, verify that all VCs are valid
    var filters = new ArrayList<>(List.of(
        new IsNotExpired(clock), new HasValidSubjectIds(issuer),
        new IsRevoked(null), new HasValidIssuer(getTrustedIssuerIds())));
    ...
    var results =
        credentials.stream()
            .map(
                ...
                -> filters.stream()
                       .reduce(
                           t -> Result.success(), CredentialValidationRule::and)
                       .apply(c))
            .reduce(Result::merge);
    return results.orElseGet(
        () -> failure("Could not determine the status of the VC validation"));
  }

  @NotNull
  private Result<ClaimToken>
  extractClaimToken(List<VerifiableCredential> credentials, String issuer) {
  ...
  private Collection<? extends CredentialValidationRule>
  getAdditionalValidations() {
    return List.of();
  }

  private List<String> getTrustedIssuerIds() {
    return trustedIssuerRegistry.getTrustedIssuers()
        .stream()
        .map(Issuer::id)
        .toList();
  }
  ...
  private Result<Void> validateScope(String scope) {
    ...
    return scope.matches(SCOPE_STRING_REGEX)
        ? success()
        : failure(
              "Scope string invalid: '%s' does not match regex %s".formatted(
                  scope, SCOPE_STRING_REGEX));
  }
}
