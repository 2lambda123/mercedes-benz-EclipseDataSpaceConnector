...
class IdentityAndTrustServiceTest {
  ...
  private final CredentialServiceUrlResolver credentialServiceUrlResolverMock =
      mock();
  private final IdentityAndTrustService service = new IdentityAndTrustService(
      mockedSts, EXPECTED_OWN_DID, EXPECTED_PARTICIPANT_ID, mockedVerifier,
      mockedClient, jwtValidatorMock, jwtVerfierMock, trustedIssuerRegistryMock,
      Clock.systemUTC(), credentialServiceUrlResolverMock, i -> i);

  @BeforeEach
  void setup() {
    when(credentialServiceUrlResolverMock.resolve(any()))
        .thenReturn(success("foobar"));
    var jwt = createJwt(
        new JWTClaimsSet.Builder().claim("scope", "foo-scope").build());
    when(jwtValidatorMock.validateToken(any(), any()))
        .thenReturn(
            success(ClaimToken.Builder.newInstance()
                        .claim("iss", CONSUMER_DID)
                        .claim("client_id", "sender-id")
                        .claim(PRESENTATION_ACCESS_TOKEN_CLAIM, jwt.getToken())
                        .build()));
    when(jwtVerfierMock.verify(any(), any())).thenReturn(success());
    when(mockedSts.createToken(any(), any()))
        .thenReturn(success(TokenRepresentation.Builder.newInstance().build()));
  }
  ...
  class ObtainClientCredentials {
    ...
    @ValueSource(strings = {"org.eclipse.edc:TestCredential:modify",
                            "org.eclipse.edc:TestCredential:",
                            "org.eclipse.edc:TestCredential: ",
                            "org.eclipse.edc:TestCredential:write*",
                            ":TestCredential:read",
                            "org.eclipse.edc:fooCredential:+"})
    void
    obtainClientCredentials_invalidScopeString(String scope) {
    ...
    @EmptySource
    void
    obtainClientCredentials_validScopeString(String scope) {
      var tp = TokenParameters.Builder.newInstance()
                   .scope(scope)
                   .audience("test-audience")
                   .build();
    ...
    void obtainClientCredentials_stsFails() {
      ...
      var tp = TokenParameters.Builder.newInstance()
                   ...
                   .build();
      when(mockedSts.createToken(any(), any()))
          .thenReturn(success(createJwt()));
      assertThat(service.obtainClientCredentials(tp)).isSucceeded();
      verify(mockedSts).createToken(
          argThat(m
                  -> m.get("iss").equals(EXPECTED_OWN_DID) &&
                         m.get("sub").equals(EXPECTED_OWN_DID) &&
                         m.get("aud").equals(tp.getAudience()) &&
                         m.get("client_id").equals(EXPECTED_PARTICIPANT_ID)),
          eq(scope));
    }
  }
  ...
  class VerifyJwtToken {
    ...
    void presentationRequestFails() {
      when(mockedClient.requestPresentation(any(), any(), any()))
    ...
    void cryptographicError() {
      when(mockedVerifier.verifyPresentation(any()))
          .thenReturn(Result.failure("Cryptographic error"));
      when(mockedClient.requestPresentation(any(), any(), any()))
          .thenReturn(success(List.of(createPresentationContainer())));
      var token = createJwt();
      var result = service.verifyJwtToken(token, verificationContext());
          ...
          createPresentationBuilder()
              ...
              .credentials(List.of(
                  createCredentialBuilder()
                      ...
                      .build()))
              .build();
      var vpContainer = new VerifiablePresentationContainer(
          "test-vp", CredentialFormat.JSON_LD, presentation);
      when(mockedVerifier.verifyPresentation(any())).thenReturn(success());
      when(mockedClient.requestPresentation(any(), any(), any()))
          .thenReturn(success(List.of(vpContainer)));
    ...
    void oneInvalidSubjectId() {
      ...
      assertThat(result)
          ...
          .hasSizeGreaterThanOrEqualTo(1)
          .contains(
              "Not all subject IDs match the expected subject ID %s".formatted(
                  CONSUMER_DID));
    }
    ...
    void credentialHasInvalidIssuer_issuerIsUrl() {
      ...
      var presentation =
          createPresentationBuilder()
              .type("VerifiablePresentation")
              .credentials(
                  List.of(createCredentialBuilder()
      ...
      assertThat(result)
          ...
          .hasSizeGreaterThanOrEqualTo(1)
          .contains(
              "Issuer 'invalid-issuer' is not in the list of trusted issuers");
    }
    ...
    void jwtTokenNotVerified() {
      when(jwtVerfierMock.verify(any(), any()))
          .thenReturn(failure("test-failure"));
      var token = createJwt();
      assertThat(service.verifyJwtToken(token, verificationContext()))
          .isFailed()
    ...
    void verify_singlePresentation_singleCredential() {
      ...
      when(mockedClient.requestPresentation(any(), any(), any()))
          .thenReturn(success(List.of(vpContainer)));
      when(trustedIssuerRegistryMock.getTrustedIssuers())
          .thenReturn(Set.of(TRUSTED_ISSUER));
      var token = createJwt(CONSUMER_DID, EXPECTED_OWN_DID);
      var result = service.verifyJwtToken(token, verificationContext());
      assertThat(result).isSucceeded().satisfies(
          ct
          -> Assertions.assertThat(ct.getClaims())
      ...
      var presentation =
          createPresentationBuilder()
              ...
              .credentials(List.of(
                  createCredentialBuilder()
                      .credentialSubjects(
                          List.of(CredentialSubject.Builder.newInstance()
                                      .id(CONSUMER_DID)
                                      .claim("some-claim", "some-val")
                                      .build()))
                      .build(),
          ...
          createPresentationBuilder()
              ...
              .credentials(List.of(
                  ...
                  createCredentialBuilder()
                      ...
                      .build()))
              .build();
      var vpContainer1 = new VerifiablePresentationContainer(
          "test-vp", CredentialFormat.JSON_LD, presentation1);
          ...
          createPresentationBuilder()
              ...
              .credentials(List.of(
                  ...
                  createCredentialBuilder()
                      ...
                      .build()))
              .build();
      var vpContainer2 = new VerifiablePresentationContainer(
          "test-vp", CredentialFormat.JSON_LD, presentation2);

      when(mockedVerifier.verifyPresentation(any())).thenReturn(success());
      when(mockedClient.requestPresentation(any(), any(), any()))
          .thenReturn(success(List.of(vpContainer1, vpContainer2)));
      when(trustedIssuerRegistryMock.getTrustedIssuers())
          .thenReturn(Set.of(TRUSTED_ISSUER));
