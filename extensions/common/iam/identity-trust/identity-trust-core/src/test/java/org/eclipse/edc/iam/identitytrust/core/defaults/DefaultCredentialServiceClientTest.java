...
class DefaultCredentialServiceClientTest {
  ...
  void setup() {
    ...
    client = new DefaultCredentialServiceClient(
        httpClientMock, Json.createBuilderFactory(Map.of()),
  ...
  void requestPresentation_singleLdpVp() throws IOException {
    when(httpClientMock.execute(any()))
        .thenReturn(response(
            200, getResourceFileContentAsString("single_ldp-vp.json")));

    var result = client.requestPresentation(CS_URL, "foo", List.of());
    assertThat(result.succeeded()).isTrue();
    assertThat(result.getContent())
        .hasSize(1)
        .allMatch(vpc -> vpc.format() == CredentialFormat.JSON_LD);
    verify(httpClientMock)
        .execute(
            argThat(rq -> rq.url().toString().endsWith("/presentation/query")));
  }
  ...
  void requestPresentation_singleJwtVp() throws IOException {
    when(httpClientMock.execute(any()))
        .thenReturn(response(
            200, getResourceFileContentAsString("single_jwt-vp.json")));

    var result = client.requestPresentation(CS_URL, "foo", List.of());
    assertThat(result.succeeded()).isTrue();
    assertThat(result.getContent())
        .hasSize(1)
        .allMatch(vpc -> vpc.format() == CredentialFormat.JWT);
    verify(httpClientMock)
        .execute(
            argThat(rq -> rq.url().toString().endsWith("/presentation/query")));
  }
  ...
  void requestPresentationLdp_multipleVp_mixed() throws IOException {
    when(httpClientMock.execute(any()))
        .thenReturn(response(200, getResourceFileContentAsString(
                                      "multiple_vp-token_mixed.json")));
    ...
    assertThat(result.getContent())
        .hasSize(2)
        .anySatisfy(
            vp -> assertThat(vp.format()).isEqualTo(CredentialFormat.JSON_LD))
        .anySatisfy(
            vp -> assertThat(vp.format()).isEqualTo(CredentialFormat.JWT));
    verify(httpClientMock)
        .execute(
            argThat(rq -> rq.url().toString().endsWith("/presentation/query")));
  }
  ...
  void requestPresentation_mulipleVp_onlyLdp() throws IOException {
    when(httpClientMock.execute(any()))
        .thenReturn(response(
            200, getResourceFileContentAsString("multiple_vp-token_ldp.json")));
  ...
  void requestPresentation_mulipleVp_onlyJwt() throws IOException {
    when(httpClientMock.execute(any()))
        .thenReturn(response(
            200, getResourceFileContentAsString("multiple_vp-token_jwt.json")));
  ...
  void requestPresentation_csReturnsError(int httpCode) throws IOException {
    ...
    assertThat(res.getFailureDetail())
        .isEqualTo("Presentation Query failed: HTTP %s, message: Test failure"
                       .formatted(httpCode));
    verify(httpClientMock)
        .execute(
            argThat(rq -> rq.url().toString().endsWith("/presentation/query")));
  ...
  void requestPresentation_emptyArray() throws IOException {
    when(httpClientMock.execute(any()))
        .thenReturn(response(
            200, "{\"vp_token\":[],\"presentation_submission\":null}"));
