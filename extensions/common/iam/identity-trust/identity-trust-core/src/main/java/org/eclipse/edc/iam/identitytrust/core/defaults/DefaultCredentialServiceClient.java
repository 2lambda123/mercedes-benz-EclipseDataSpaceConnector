...
public class DefaultCredentialServiceClient implements CredentialServiceClient {
  ...
  public DefaultCredentialServiceClient(
      EdcHttpClient httpClient, JsonBuilderFactory jsonFactory,
      ObjectMapper jsonLdMapper, TypeTransformerRegistry transformerRegistry,
      JsonLd jsonLd, Monitor monitor) {
    this.httpClient = httpClient;
    this.jsonFactory = jsonFactory;
    this.objectMapper = jsonLdMapper.enable(
  ...
  @Override
  public Result<List<VerifiablePresentationContainer>>
  requestPresentation(String credentialServiceBaseUrl,
                      String selfIssuedTokenJwt, List<String> scopes) {
    ...
    try {
      ...
      if (response.isSuccessful() && response.body() != null) {
        var presentationResponse =
            objectMapper.readValue(body, PresentationResponse.class);
        return parseResponse(presentationResponse);
      }
      return failure(
          "Presentation Query failed: HTTP %s, message: %s".formatted(
              response.code(), body));
  ...
  private Result<List<VerifiablePresentationContainer>>
  parseResponse(PresentationResponse presentationResponse) throws IOException {
    ...
    if (vpResults.stream().anyMatch(AbstractResult::failed)) {
      return failure(
          "One or more VP tokens could not be parsed. Details: %s".formatted(
              vpResults.stream()
  ...
  private Result<VerifiablePresentationContainer> parseLdpVp(Object vpObj) {
    ...
    return jsonLd.expand(jsonObj)
        .compose(expanded
                 -> transformerRegistry.transform(expanded,
                                                  VerifiablePresentation.class))
        .map(vp
             -> new VerifiablePresentationContainer(
                 rawStr, CredentialFormat.JSON_LD, vp));
  }
  ...
  private Result<VerifiablePresentationContainer> parseJwtVp(String rawJwt) {
    return transformerRegistry.transform(rawJwt, VerifiablePresentation.class)
        .map(pres
             -> new VerifiablePresentationContainer(
                 rawJwt, CredentialFormat.JWT, pres));
  }
