...
class DapsIntegrationTest {
  ...
  private static final String AUDIENCE_IDS_CONNECTORS_ALL =
      "idsc:IDS_CONNECTORS_ALL";
  private static final String CLIENT_ID =
      "68:99:2E:D4:13:2D:FD:3A:66:6B:85:DE:FB:98:2E:2D:FD:E7:83:D7";
  private static final String CLIENT_KEYSTORE_KEY_ALIAS = "1";
  private static final String CLIENT_KEYSTORE_PASSWORD = "1234";

  private final Path resourceFolder = findBuildRoot().toPath().resolve(
      "extensions/common/iam/oauth2/oauth2-daps/src/test/resources");

  @Container
  private final GenericContainer<?> daps =
      new GenericContainer<>("ghcr.io/fraunhofer-aisec/omejdn-server:1.4.2")
          .withExposedPorts(4567)
          .withFileSystemBind(resourceFolder.resolve("config").toString(),
                              "/opt/config")
          .withFileSystemBind(resourceFolder.resolve("keys").toString(),
                              "/opt/keys");

  @BeforeEach
  protected void before(EdcExtension extension) {
    System.setProperty("edc.vault",
                       "src/test/resources/empty-vault.properties");
    ...
    var dapsUrl =
        "http://%s:%s".formatted(daps.getHost(), daps.getFirstMappedPort());

    extension.setConfiguration(Map.of(
        "edc.oauth.token.url", dapsUrl + "/token", "edc.oauth.client.id",
        CLIENT_ID, "edc.oauth.provider.audience", AUDIENCE_IDS_CONNECTORS_ALL,
  ...
  void retrieveTokenAndValidate(IdentityService identityService) {
    ...
    assertThat(tokenResult.succeeded())
        .withFailMessage(tokenResult::getFailureDetail)
        .isTrue();
    ...
    var verificationResult = identityService.verifyJwtToken(
        tokenResult.getContent(), verificationContext);

    assertThat(verificationResult.succeeded())
        .withFailMessage(verificationResult::getFailureDetail)
        .isTrue();
  }
}
