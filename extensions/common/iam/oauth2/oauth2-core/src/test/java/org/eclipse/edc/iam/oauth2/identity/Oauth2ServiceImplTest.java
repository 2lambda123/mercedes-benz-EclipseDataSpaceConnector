...
class Oauth2ServiceImplTest {
  ...
  private static final VerificationContext VERIFICATION_CONTEXT =
  ...
  void setUp() throws JOSEException {
    ...
    when(publicKeyResolverMock.resolveKey(anyString()))
        .thenReturn(testKey.toPublicKey());
    ...
    var validationRulesRegistry =
        new Oauth2ValidationRulesRegistryImpl(configuration, clock);
    var tokenValidationService = new TokenValidationServiceImpl(
        publicKeyResolverMock, validationRulesRegistry);

    var jwtDecoratorRegistry = new JwtDecoratorRegistryImpl();
    jwtDecoratorRegistry.register(jwtDecorator);

    authService = new Oauth2ServiceImpl(
        configuration, tokenGenerationService, client, jwtDecoratorRegistry,
        tokenValidationService, credentialsRequestAdditionalParametersProvider);
  }

  @Test
  void obtainClientCredentials() {
    when(credentialsRequestAdditionalParametersProvider.provide(any()))
        .thenReturn(emptyMap());
    when(tokenGenerationService.generate(any()))
        .thenReturn(Result.success(TokenRepresentation.Builder.newInstance()
    ...
    assertThat(captured).isNotNull().isInstanceOf(
        PrivateKeyOauth2CredentialsRequest.class);
    ...
    assertThat(capturedRequest.getClientAssertion())
        .isEqualTo("assertionToken");
    assertThat(capturedRequest.getClientAssertionType())
  ...
  void obtainClientCredentials_addsAdditionalFormParameters() {
    when(credentialsRequestAdditionalParametersProvider.provide(any()))
        .thenReturn(Map.of("parameterKey", "parameterValue"));
    when(tokenGenerationService.generate(any()))
        .thenReturn(Result.success(TokenRepresentation.Builder.newInstance()
                                       .token("assertionToken")
                                       .build()));
    ...
    assertThat(capturedRequest.getClientAssertion())
        .isEqualTo("assertionToken");
    assertThat(capturedRequest.getParams())
        .containsEntry("parameterKey", "parameterValue");
  }
  ...
  void verifyNoAudienceToken() {
    var jwt = createJwt(null, Date.from(now.minusSeconds(1000)),
                        Date.from(now.plusSeconds(1000)));
  ...
  void verifyInvalidAudienceToken() {
    var jwt = createJwt("different.audience", Date.from(now.minusSeconds(1000)),
                        Date.from(now.plusSeconds(1000)));
  ...
  void verifyInvalidAttemptUseNotBeforeToken() {
    var jwt = createJwt(PROVIDER_AUDIENCE, Date.from(now.plusSeconds(1000)),
                        Date.from(now.plusSeconds(1000)));
  ...
  void verifyExpiredToken() {
    var jwt = createJwt(PROVIDER_AUDIENCE, Date.from(now.minusSeconds(1000)),
  ...
  @Test
  void verifyValidJwt() {
    var jwt = createJwt(ENDPOINT_AUDIENCE, Date.from(now.minusSeconds(1000)),
    ...
    assertThat(result.getContent().getClaims())
        .hasSize(3)
        .containsKeys(AUDIENCE, NOT_BEFORE, EXPIRATION_TIME);
  }

  private PrivateKeyOauth2CredentialsRequest
  createRequest(Map<String, String> additional) {
  ...
  private TokenRepresentation createJwt(String aud, Date nbf, Date exp) {
    var claimsSet = new JWTClaimsSet.Builder()
                        ...
                        .build();
    var header =
        new JWSHeader.Builder(JWSAlgorithm.RS256).keyID("an-id").build();
