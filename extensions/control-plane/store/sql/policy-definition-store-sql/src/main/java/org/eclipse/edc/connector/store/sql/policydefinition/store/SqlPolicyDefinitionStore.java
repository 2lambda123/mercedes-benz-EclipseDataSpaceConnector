...
public class SqlPolicyDefinitionStore
    extends AbstractSqlStore implements PolicyDefinitionStore {

  private final SqlPolicyStoreStatements statements;
  private final TypeReference<List<Permission>> permissionListType =
      new TypeReference<>() {};
  private final TypeReference<List<Prohibition>> prohibitionListType =
      new TypeReference<>() {};
  private final TypeReference<List<Duty>> dutyListType =
      new TypeReference<>() {};
  private final TypeReference<PolicyType> policyType = new TypeReference<>() {};
  private final TypeReference<Map<String, Object>> extensiblePropertiesType =
      new TypeReference<>() {};

  private static final TypeReference<Map<String, Object>>
      PRIVATE_PROPERTIES_TYPE = new TypeReference<>() {};

  public SqlPolicyDefinitionStore(
      ...
      SqlPolicyStoreStatements sqlPolicyStoreStatements,
  ...
  public PolicyDefinition findById(String id) {
    return transactionContext.execute(() -> {
      var query = QuerySpec.Builder.newInstance()
                      .filter(List.of(new Criterion("id", "=", id)))
                      .build();
      try {
        var queryStatement = statements.createQuery(query);
        return queryExecutor.single(getConnection(), true, this::mapResultSet,
                                    queryStatement.getQueryAsString(),
    ...
    return transactionContext.execute(() -> {
      if (findById(policy.getUid()) != null) {
        return StoreResult.alreadyExists(
            format(POLICY_ALREADY_EXISTS, policyId));
      } else {
        insert(policy);
  ...
  @Override
  public StoreResult<PolicyDefinition>
  update(PolicyDefinition policyDefinition) {
  ...
  public StoreResult<PolicyDefinition> delete(String policyId) {
    ...
    return transactionContext.execute(() -> {
      try (var connection = getConnection()) {
        ...
        if (entity != null) {
          queryExecutor.execute(connection, statements.getDeleteTemplate(),
                                policyId);
    ...
    transactionContext.execute(() -> {
      try (var connection = getConnection()) {
        ...
        queryExecutor.execute(
            connection, statements.getInsertTemplate(), id,
            ...
            toJson(policy.getExtensibleProperties()), policy.getInheritsFrom(),
            policy.getAssigner(), policy.getAssignee(), policy.getTarget(),
            toJson(policy.getType(), policyType), def.getCreatedAt(),
  ...
  private void updateInternal(PolicyDefinition def) {
    transactionContext.execute(() -> {
      try (var connection = getConnection()) {
        ...
        var id = def.getUid();
        queryExecutor.execute(
            connection, statements.getUpdateTemplate(),
            toJson(policy.getPermissions(), permissionListType),
            toJson(policy.getProhibitions(), prohibitionListType),
            toJson(policy.getObligations(), dutyListType),
            toJson(policy.getExtensibleProperties()), policy.getInheritsFrom(),
            ...
            toJson(def.getPrivateProperties()), id);
      } catch (Exception e) {
        throw new EdcPersistenceException(e.getMessage(), e);
      }
    });
  ...
  private PolicyDefinition mapResultSet(ResultSet resultSet)
      throws SQLException {
    var policy =
        Policy.Builder.newInstance()
            .permissions(
                fromJson(resultSet.getString(statements.getPermissionsColumn()),
                         permissionListType))
            .prohibitions(fromJson(
                resultSet.getString(statements.getProhibitionsColumn()),
                prohibitionListType))
            .duties(fromJson(resultSet.getString(statements.getDutiesColumn()),
                             dutyListType))
            .extensibleProperties(fromJson(
                resultSet.getString(statements.getExtensiblePropertiesColumn()),
                extensiblePropertiesType))
            .inheritsFrom(
            ...
            .type(fromJson(resultSet.getString(statements.getTypeColumn()),
                           policyType))
            .build();
    ...
    return PolicyDefinition.Builder.newInstance()
        ...
        .privateProperties(fromJson(
            resultSet.getString(statements.getPrivatePropertiesColumn()),
            PRIVATE_PROPERTIES_TYPE))
        .build();
  }
}
