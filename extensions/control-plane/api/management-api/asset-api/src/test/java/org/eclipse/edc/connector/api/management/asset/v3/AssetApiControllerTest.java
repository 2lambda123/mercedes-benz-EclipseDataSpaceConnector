...
class AssetApiControllerTest extends RestControllerTestBase {
  ...
  private final JsonObjectValidatorRegistry validator =
      mock(JsonObjectValidatorRegistry.class);

  @BeforeEach
  void setup() {
    when(transformerRegistry.transform(isA(JsonObject.class),
                                       eq(DataAddress.class)))
        .thenReturn(Result.success(
            DataAddress.Builder.newInstance().type("test-type").build()));
    when(transformerRegistry.transform(isA(IdResponse.class),
                                       eq(JsonObject.class)))
        .thenAnswer(a -> {
  ...
  void requestAsset() {
    ...
    verify(transformerRegistry)
        .transform(isA(Asset.class), eq(JsonObject.class));
    verify(transformerRegistry)
        .transform(isA(JsonObject.class), eq(QuerySpec.class));
  }
  ...
  void requestAsset_shouldReturnBadRequest_whenValidationFails() {
    ...
    when(validator.validate(any(), any()))
        .thenReturn(ValidationResult.failure(
            violation("validation failure", "a path")));
    ...
    baseRequest()
        ...
        .then()
        .statusCode(400);
    verify(validator).validate(eq(QuerySpec.EDC_QUERY_SPEC_TYPE),
  ...
  void getSingleAsset() {
    ...
    when(transformerRegistry.transform(isA(Asset.class), eq(JsonObject.class)))
        .thenReturn(Result.success(assetJson));
  ...
  void getSingleAsset_notFound() {
    ...
    baseRequest().get("/assets/not-existent-id").then().statusCode(404);
  }

  @Test
  void getAssetById_shouldReturnNotFound_whenTransformFails() {
    when(service.findById("id"))
        .thenReturn(Asset.Builder.newInstance().build());
    when(transformerRegistry.transform(isA(Asset.class), eq(JsonObject.class)))
        .thenReturn(Result.failure("failure"));

    baseRequest().get("/assets/id").then().statusCode(500);
  }
  ...
  void createAsset() {
    var asset =
        createAssetBuilder()
            ...
            .build();
    when(transformerRegistry.transform(any(JsonObject.class), eq(Asset.class)))
        .thenReturn(Result.success(asset));
    when(service.create(isA(Asset.class)))
        .thenReturn(ServiceResult.success(asset));
    when(validator.validate(any(), any()))
        .thenReturn(ValidationResult.success());
    ...
    verify(transformerRegistry)
        .transform(isA(IdResponse.class), eq(JsonObject.class));
  ...
  void createAsset_shouldReturnBadRequest_whenValidationFails() {
    when(validator.validate(any(), any()))
        .thenReturn(ValidationResult.failure(violation("a failure", "a path")));
  ...
  void createAsset_shouldReturnBadRequest_whenTransformFails() {
    when(transformerRegistry.transform(isA(JsonObject.class), eq(Asset.class)))
  ...
  void createAsset_emptyAttributes() {
    when(transformerRegistry.transform(isA(JsonObject.class), any()))
        .thenReturn(Result.failure("Cannot be transformed"));
    when(validator.validate(any(), any()))
        .thenReturn(ValidationResult.success());
  ...
  void deleteAsset_conflicts() {
    ...
    baseRequest().contentType(JSON).delete("/assets/id").then().statusCode(409);
  }
  ...
  void updateAsset_shouldReturnNotFound_whenItDoesNotExists() {
    ...
    when(transformerRegistry.transform(isA(JsonObject.class), eq(Asset.class)))
        .thenReturn(Result.success(asset));
    when(service.update(any(Asset.class)))
  ...
  void updateAsset_shouldReturnBadRequest_whenValidationFails() {
    ...
    when(validator.validate(any(), any()))
        .thenReturn(
            ValidationResult.failure(violation("validation failure", "path")));
  ...
  protected Object controller() {
    return new AssetApiController(service, transformerRegistry, monitor,
                                  validator);
  }
  ...
  private RequestSpecification baseRequest() {
    return given().baseUri("http://localhost:" + port + "/v3").when();
  }
