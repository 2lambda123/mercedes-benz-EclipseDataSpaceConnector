...
class ContractAgreementApiControllerTest extends RestControllerTestBase {
  ...
  void queryAllAgreements_whenExists() {
    ...
    when(service.search(any(QuerySpec.class)))
        .thenReturn(ServiceResult.success(List.of(
            createContractAgreement("id1"), createContractAgreement("id2"))));
    when(transformerRegistry.transform(any(ContractAgreement.class),
                                       eq(JsonObject.class)))
        .thenReturn(Result.success(expanded));
  ...
  void queryAllAgreements_whenNoneExists() {
    ...
    when(transformerRegistry.transform(any(JsonObject.class),
        ...
        .thenReturn(Result.success(QuerySpec.none()));
    when(service.search(any(QuerySpec.class)))
        .thenReturn(ServiceResult.success(emptyList()));
  ...
  void queryAllAgreements_shouldReturnBadRequest_whenValidationFails() {
    when(validatorRegistry.validate(any(), any()))
        .thenReturn(ValidationResult.failure(
            Violation.violation("failure", "failing path")));
  ...
  void getContractAgreement() {
    when(service.findById(eq("id1")))
        .thenReturn(createContractAgreement("id1"));
    when(transformerRegistry.transform(any(ContractAgreement.class),
                                       eq(JsonObject.class)))
        .thenReturn(Result.success(Json.createObjectBuilder().build()));

    baseRequest().contentType(JSON).get("/id1").then().statusCode(200).body(
        notNullValue());
  ...
  void getContractAgreement_notFound() {
    ...
    baseRequest().contentType(JSON).get("/id1").then().statusCode(404).body(
        notNullValue());
  ...
  void getContractAgreement_transformationFails() {
    ...
    baseRequest().contentType(JSON).get("/id1").then().statusCode(500);
  ...
  void getContractNegotiation() {
    when(service.findNegotiation(any()))
        .thenReturn(createContractNegotiationBuilder("negotiation-id").build());
  ...
  void getContractNegotiation_transformationFails() {
    when(service.findNegotiation(any()))
        .thenReturn(createContractNegotiationBuilder("agreement-id").build());
    ...
    verify(service).findNegotiation("agreement-id");
    verify(transformerRegistry)
        .transform(isA(ContractNegotiation.class), eq(JsonObject.class));
    verifyNoMoreInteractions(service, transformerRegistry);
  }

  @Override
  protected Object controller() {
    return new ContractAgreementApiController(service, transformerRegistry,
