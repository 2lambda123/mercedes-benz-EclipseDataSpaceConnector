...
public class TransferProcessApiController implements TransferProcessApi {
  ...
  public TransferProcessApiController(
      Monitor monitor, TransferProcessService service,
  ...
  public JsonArray queryTransferProcesses(JsonObject querySpecJson) {
    ...
    return service.search(querySpec)
        ...
        .stream()
        .map(transferProcess
             -> transformerRegistry.transform(transferProcess, JsonObject.class)
  ...
  public JsonObject getTransferProcess(@PathParam("id") String id) {
    ...
    return transformerRegistry.transform(definition, JsonObject.class)
        .onFailure(f -> monitor.warning(f.getFailureDetail()))
        .orElseThrow(
            failure -> new ObjectNotFoundException(TransferProcess.class, id));
  }
  ...
  public JsonObject getTransferProcessState(@PathParam("id") String id) {
    return Optional.of(id)
        ...
        .map(state
             -> transformerRegistry.transform(state, JsonObject.class)
                    .onFailure(f -> monitor.warning(f.getFailureDetail()))
                    .orElseThrow(failure
                                 -> new ObjectNotFoundException(
                                     TransferProcess.class, id)))
        .orElseThrow(
            () -> new ObjectNotFoundException(TransferProcess.class, id));
  }

  @POST
  @Override
  public JsonObject initiateTransferProcess(JsonObject request) {
    validatorRegistry.validate(TRANSFER_REQUEST_TYPE, request)
        .orElseThrow(ValidationFailureException::new);

    var transferRequest =
        transformerRegistry.transform(request, TransferRequest.class)
            .orElseThrow(InvalidRequestException::new);
    ...
    var createdTransfer =
        service.initiateTransfer(transferRequest)
            .onSuccess(d
                       -> monitor.debug(
                           format("Transfer Process created %s", d.getId())))
  ...
  public void deprovisionTransferProcess(@PathParam("id") String id) {
    service.deprovision(id)
        .onSuccess(
            tp
            -> monitor.debug(format(
                "Deprovision requested for TransferProcess with ID %s", id)))
  ...
  @Path("/{id}/terminate")
  @Override
  public void terminateTransferProcess(@PathParam("id") String id,
                                       JsonObject requestBody) {
    validatorRegistry.validate(TERMINATE_TRANSFER_TYPE, requestBody)
        .orElseThrow(ValidationFailureException::new);

    var terminateTransfer =
        transformerRegistry.transform(requestBody, TerminateTransfer.class)
            .orElseThrow(InvalidRequestException::new);

    service
        .terminate(new TerminateTransferCommand(id, terminateTransfer.reason()))
        .onSuccess(
            tp
            -> monitor.debug(format(
                "Termination requested for TransferProcess with ID %s", id)))
        .orElseThrow(
            failure -> mapToException(failure, TransferProcess.class, id));
  }
}
