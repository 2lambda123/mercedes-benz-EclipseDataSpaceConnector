...
class TransferProcessApiControllerTest extends RestControllerTestBase {
  ...
  @Test
  void search_shouldReturnQueriedTransferProcesses() {
    var querySpec = QuerySpec.none();
    var expandedRequestBody = Json.createObjectBuilder().build();
    var transferProcess = createTransferProcess().id("id").build();
    var expandedResponseBody = Json.createObjectBuilder()
                                   .add("id", "id")
                                   .add("createdAt", 1234)
                                   .build();
    when(validatorRegistry.validate(any(), any()))
        .thenReturn(ValidationResult.success());
    when(transformerRegistry.transform(any(), eq(QuerySpec.class)))
        .thenReturn(Result.success(querySpec));
    when(service.search(any()))
        .thenReturn(ServiceResult.success(List.of(transferProcess)));
    when(transformerRegistry.transform(any(), eq(JsonObject.class)))
        .thenReturn(Result.success(expandedResponseBody));
  ...
  @Test
  void search_shouldNotReturnError_whenEmptyBody() {
    var querySpec = QuerySpec.none();
  ...
  @Test
  void search_shouldReturnBadRequest_whenValidationFails() {
    when(validatorRegistry.validate(any(), any()))
  ...
  @Test
  void search_shouldReturn400_whenQuerySpecTransformFails() {
    when(validatorRegistry.validate(any(), any()))
        .thenReturn(ValidationResult.success());
    when(transformerRegistry.transform(any(), eq(QuerySpec.class)))
        .thenReturn(Result.failure("error"));
  ...
  void initiate_shouldReturnId() {
    ...
    var transferProcess = createTransferProcess().id("id").build();
    var responseBody =
        Json.createObjectBuilder().add(ID, "transferProcessId").build();
    when(validatorRegistry.validate(any(), any()))
        .thenReturn(ValidationResult.success());
    when(transformerRegistry.transform(any(), eq(TransferRequest.class)))
        .thenReturn(Result.success(transferRequest));
    when(service.initiateTransfer(any()))
    ...
    verify(transformerRegistry)
        .transform(isA(JsonObject.class), eq(TransferRequest.class));
  ...
  void terminate() {
    ...
    when(validatorRegistry.validate(any(), any()))
        .thenReturn(ValidationResult.success());
    when(transformerRegistry.transform(any(), eq(TerminateTransfer.class)))
        .thenReturn(Result.success(terminateTransfer));
