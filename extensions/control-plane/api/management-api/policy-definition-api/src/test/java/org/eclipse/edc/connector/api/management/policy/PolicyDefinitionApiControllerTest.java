...
class PolicyDefinitionApiControllerTest extends RestControllerTestBase {
  ...
  void create_shouldReturnDefinitionId() {
    ...
    var policyDefinition = createPolicyDefinition()
    ...
    when(transformerRegistry.transform(any(), eq(PolicyDefinition.class)))
        .thenReturn(Result.success(policyDefinition));
    when(service.create(any()))
        .thenReturn(ServiceResult.success(policyDefinition));
    ...
    verify(transformerRegistry)
        .transform(isA(JsonObject.class), eq(PolicyDefinition.class));
  ...
  void create_shouldReturnBadRequest_whenValidationFails() {
    when(validatorRegistry.validate(any(), any()))
        .thenReturn(
            ValidationResult.failure(violation("failure", "failure path")));
    var requestBody = Json.createObjectBuilder()
                          .add("policy", Json.createObjectBuilder()
                                             .add(CONTEXT, "context")
                                             .add(TYPE, "Set")
  ...
  void search_shouldReturnQueriedPolicyDefinitions() {
    ...
    when(service.search(any()))
        .thenReturn(ServiceResult.success(List.of(policyDefinition)));
    when(transformerRegistry.transform(any(), eq(JsonObject.class)))
  ...
  void search_shouldReturn400_whenInvalidQuery() {
    var requestBody = Json.createObjectBuilder().add("offset", -1).build();
    when(validatorRegistry.validate(any(), any()))
        .thenReturn(ValidationResult.success());
  ...
  protected Object controller() {
    return new PolicyDefinitionApiController(monitor, transformerRegistry,
                                             service, validatorRegistry);
  }
