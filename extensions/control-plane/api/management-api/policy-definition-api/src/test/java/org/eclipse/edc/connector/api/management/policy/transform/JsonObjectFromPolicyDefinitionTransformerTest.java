...
class JsonObjectFromPolicyDefinitionTransformerTest {
  ...
  private final JsonObjectFromPolicyDefinitionTransformer transformer =
      new JsonObjectFromPolicyDefinitionTransformer(
          Json.createBuilderFactory(emptyMap()), createObjectMapper());
  ...
  void transform() {
    ...
    var input = PolicyDefinition.Builder.newInstance()
                    .id("definitionId")
                    .policy(policy)
                    .build();
    ...
    assertThat(result.getJsonObject(EDC_POLICY_DEFINITION_POLICY))
        .isSameAs(policyJson);
