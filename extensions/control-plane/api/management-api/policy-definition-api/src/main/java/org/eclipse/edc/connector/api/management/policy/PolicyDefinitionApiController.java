...
public class PolicyDefinitionApiController implements PolicyDefinitionApi {
  ...
  public PolicyDefinitionApiController(
      Monitor monitor, TypeTransformerRegistry transformerRegistry,
  ...
  public JsonArray queryPolicyDefinitions(JsonObject querySpecJson) {
    ...
    return service.search(querySpec)
        ...
        .map(policyDefinition
             -> transformerRegistry.transform(policyDefinition,
                                              JsonObject.class))
  ...
  public JsonObject getPolicyDefinition(@PathParam("id") String id) {
    ...
    return transformerRegistry.transform(definition, JsonObject.class)
        .orElseThrow(
            failure -> new ObjectNotFoundException(PolicyDefinition.class, id));
  }
  ...
  public JsonObject createPolicyDefinition(JsonObject request) {
    ...
    var definition =
        transformerRegistry.transform(request, PolicyDefinition.class)
            .orElseThrow(InvalidRequestException::new);
    ...
    var createdDefinition =
        service.create(definition)
            .onSuccess(d
                       -> monitor.debug(
                           format("Policy Definition created %s", d.getId())))
            .orElseThrow(
                exceptionMapper(PolicyDefinition.class, definition.getId()));
  ...
  public void deletePolicyDefinition(@PathParam("id") String id) {
    service.deleteById(id)
        .onSuccess(
            d
            -> monitor.debug(format("Policy Definition deleted %s", d.getId())))
  ...
  @Path("{id}")
  @Override
  public void updatePolicyDefinition(@PathParam("id") String id,
                                     JsonObject input) {
    validatorRegistry.validate(EDC_POLICY_DEFINITION_TYPE, input)
        .orElseThrow(ValidationFailureException::new);

    var policyDefinition =
        transformerRegistry.transform(input, PolicyDefinition.class)
            .orElseThrow(InvalidRequestException::new);

    service.update(policyDefinition)
        .onSuccess(
            d
            -> monitor.debug(format("Policy Definition updated %s", d.getId())))
        .orElseThrow(exceptionMapper(PolicyDefinition.class, id));
  }
}
