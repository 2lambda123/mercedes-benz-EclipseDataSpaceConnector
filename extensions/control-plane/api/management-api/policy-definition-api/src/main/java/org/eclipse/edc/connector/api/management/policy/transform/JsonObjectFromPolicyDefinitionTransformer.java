...
public class JsonObjectFromPolicyDefinitionTransformer
    extends AbstractJsonLdTransformer<PolicyDefinition, JsonObject> {

  private final ObjectMapper mapper;
  private final JsonBuilderFactory jsonFactory;

  public JsonObjectFromPolicyDefinitionTransformer(
      JsonBuilderFactory jsonFactory, ObjectMapper jsonLdMapper) {
  ...
  @Override
  public @Nullable JsonObject transform(@NotNull PolicyDefinition input,
    ...
    if (!input.getPrivateProperties().isEmpty()) {
      ...
      transformProperties(input.getPrivateProperties(), privatePropBuilder,
                          mapper, context);
      objectBuilder.add(
          PolicyDefinition.EDC_POLICY_DEFINITION_PRIVATE_PROPERTIES,
          privatePropBuilder);
    }
