...
class ContractDefinitionApiControllerTest extends RestControllerTestBase {
  ...
  void queryAllContractDefinitions(String body) {
    ...
    when(service.search(any()))
        .thenReturn(
            ServiceResult.success(List.of(createContractDefinition().build())));
    when(transformerRegistry.transform(any(JsonObject.class),
                                       eq(QuerySpec.class)))
        .thenReturn(Result.success(QuerySpec.Builder.newInstance().build()));
    when(transformerRegistry.transform(any(ContractDefinition.class),
                                       eq(JsonObject.class)))
        .thenReturn(Result.success(createExpandedJsonObject()));
  ...
  void queryAll_serviceBadRequest() {
    ...
    var error = baseRequest()
                    ...
                    .extract()
                    .body()
                    .as(new TypeRef<List<ApiErrorDetail>>() {})
                    .get(0);
  ...
  void getContractDefById_notExists() {
    ...
    baseRequest().get("/test-id").then().statusCode(404);

    verify(service).findById("test-id");
    verify(transformerRegistry, never())
        .transform(any(ContractDefinition.class), eq(JsonObject.class));
  }
  ...
  void create() {
    ...
    var responseBody = createObjectBuilder()
                           .add(TYPE, IdResponse.ID_RESPONSE_TYPE)
                           .add(ID, entity.getId())
                           .build();
    when(validatorRegistry.validate(any(), any()))
        .thenReturn(ValidationResult.success());
    when(transformerRegistry.transform(any(JsonObject.class),
                                       eq(ContractDefinition.class)))
        .thenReturn(Result.success(entity));
    when(service.create(any(ContractDefinition.class)))
        .thenReturn(ServiceResult.success(entity));
  ...
  void create_shouldReturnBadRequest_whenValidationFails() {
    var requestJson = createObjectBuilder()
                          ...
                          .add(CONTRACT_DEFINITION_ASSETS_SELECTOR,
                               createCriterionBuilder().build())
                          .build();
    when(validatorRegistry.validate(any(), any()))
        .thenReturn(ValidationResult.failure(violation("failure", "path")));
  ...
  void create_exists() {
    ...
    when(transformerRegistry.transform(any(JsonObject.class),
        ...
        .thenReturn(Result.success(entity));
    when(service.create(any(ContractDefinition.class)))
        .thenReturn(ServiceResult.conflict("test-message"));
  ...
  void delete_exists() {
    ...
    when(service.delete(eq(contractDefinition.getId())))
        .thenReturn(ServiceResult.success(contractDefinition));

    baseRequest().delete(contractDefinition.getId()).then().statusCode(204);
  ...
  void delete_notExists() {
    var contractDefinition = createContractDefinition().build();
    when(service.delete(eq(contractDefinition.getId())))
        .thenReturn(ServiceResult.notFound("test-message"));

    baseRequest().delete(contractDefinition.getId()).then().statusCode(404);
  ...
  void delete_notPossible() {
    ...
    baseRequest().delete(contractDefinition.getId()).then().statusCode(409);
  ...
  void update_whenExists() {
    ...
    when(transformerRegistry.transform(any(JsonObject.class),
                                       eq(ContractDefinition.class)))
        .thenReturn(Result.success(entity));
    when(service.update(any(ContractDefinition.class)))
  ...
  protected Object controller() {
    return new ContractDefinitionApiController(transformerRegistry, service,
