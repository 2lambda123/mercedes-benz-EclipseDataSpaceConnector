...
class ContractNegotiationApiControllerTest extends RestControllerTestBase {
  private final ContractNegotiationService service = mock();
  private final TypeTransformerRegistry transformerRegistry = mock();
  private final JsonObjectValidatorRegistry validatorRegistry = mock();

  @Test
  void getAll() {
    when(service.search(any(QuerySpec.class)))
        .thenReturn(
            ServiceResult.success(List.of(createContractNegotiation("cn1"),
    ...
    verify(transformerRegistry, times(2))
        .transform(any(ContractNegotiation.class), eq(JsonObject.class));
  }

  @Test
  void getAll_shouldReturnBadRequest_whenValidationFails() {
    when(validatorRegistry.validate(any(), any()))
        .thenReturn(ValidationResult.failure(violation("failure", "path")));
  ...
  void getAll_queryTransformationFails() {
    ...
    when(service.search(any(QuerySpec.class)))
        .thenReturn(
            ServiceResult.success(List.of(createContractNegotiation("cn1"),
                                          createContractNegotiation("cn2"))));
    when(transformerRegistry.transform(any(JsonObject.class),
                                       eq(QuerySpec.class)))
    ...
    verify(transformerRegistry)
        .transform(any(JsonObject.class), eq(QuerySpec.class));
  ...
  void getById() {
    when(service.findbyId(anyString()))
        .thenReturn(createContractNegotiation("cn1"));
    when(transformerRegistry.transform(any(ContractNegotiation.class),
                                       eq(JsonObject.class)))
        .thenReturn(Result.success(createObjectBuilder().build()));

    baseRequest().get("/cn1").then().statusCode(200).contentType(JSON).body(
        notNullValue());
  ...
  void getById_notFound() {
    ...
    baseRequest().get("/cn1").then().statusCode(404).contentType(JSON).body(
        notNullValue());
  ...
  void getSingleContractNegotiationState() {
    ...
    when(transformerRegistry.transform(any(NegotiationState.class),
                                       eq(JsonObject.class)))
    ...
    verify(transformerRegistry)
        .transform(any(NegotiationState.class), eq(JsonObject.class));
    verifyNoMoreInteractions(service, transformerRegistry);
  }

  @Test
  void getSingleContractNegotiationAgreement() {
    when(service.getForNegotiation(eq("cn1")))
        .thenReturn(createContractAgreement("cn1"));
    when(transformerRegistry.transform(any(ContractAgreement.class),
                                       eq(JsonObject.class)))
        .thenReturn(Result.success(createObjectBuilder().build()));
    ...
    verify(transformerRegistry)
        .transform(any(ContractAgreement.class), eq(JsonObject.class));
  ...
  void getSingleContractNegotiationAgreement_transformationFails() {
    ...
    baseRequest().get("/cn1/agreement").then().statusCode(500);
  ...
  void initiate() {
    ...
    var responseBody = createObjectBuilder()
                           .add(TYPE, ID_RESPONSE_TYPE)
                           .add(ID, contractNegotiation.getId())
                           .build();

    when(transformerRegistry.transform(any(JsonObject.class),
                                       eq(ContractRequest.class)))
    ...
    when(transformerRegistry.transform(any(), eq(JsonObject.class)))
        .thenReturn(Result.success(responseBody));
    when(service.initiateNegotiation(any(ContractRequest.class)))
        .thenReturn(contractNegotiation);

    when(transformerRegistry.transform(any(IdResponse.class),
                                       eq(JsonObject.class)))
        .thenReturn(Result.success(responseBody));
    ...
    verify(transformerRegistry)
        .transform(any(JsonObject.class), eq(ContractRequest.class));
    verify(transformerRegistry)
        .transform(any(IdResponse.class), eq(JsonObject.class));
    verifyNoMoreInteractions(transformerRegistry, service);
  }
  ...
  void initiate_shouldReturnBadRequest_whenValidationFails() {
    ...
    verify(validatorRegistry)
        .validate(eq(ContractRequest.CONTRACT_REQUEST_TYPE), any());
  ...
  void initiate_invalidRequest() {
    when(validatorRegistry.validate(any(), any()))
        .thenReturn(ValidationResult.success());
    when(transformerRegistry.transform(any(JsonObject.class), any()))
        .thenReturn(Result.failure("test-failure"));
  ...
  void terminate_shouldCallService() {
    ...
    when(validatorRegistry.validate(any(), any()))
        .thenReturn(ValidationResult.success());
    when(transformerRegistry.transform(any(JsonObject.class),
                                       eq(TerminateNegotiationCommand.class)))
        .thenReturn(Result.success(command));
  ...
  void terminate_shouldReturnError_whenServiceFails() {
    ...
    when(transformerRegistry.transform(any(JsonObject.class),
                                       eq(TerminateNegotiationCommand.class)))
        .thenReturn(Result.success(command));
    when(service.terminate(any()))
        .thenReturn(ServiceResult.conflict("conflict"));
  ...
  protected Object controller() {
    return new ContractNegotiationApiController(service, transformerRegistry,
                                                monitor, validatorRegistry);
  }
  ...
  private ContractNegotiation createContractNegotiation(String negotiationId) {
    return createContractNegotiationBuilder(negotiationId).build();
  }
  ...
  private ContractNegotiation.Builder
  createContractNegotiationBuilder(String negotiationId) {
    return ContractNegotiation.Builder.newInstance()
        .id(negotiationId)
        .counterPartyId(randomUUID().toString())
        .counterPartyAddress("address")
        .callbackAddresses(List.of(
            CallbackAddress.Builder.newInstance().uri("local://test").build()))
        .protocol("protocol");
  }
}
