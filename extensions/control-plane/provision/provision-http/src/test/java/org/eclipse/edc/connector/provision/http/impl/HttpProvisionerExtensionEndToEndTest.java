...
public class HttpProvisionerExtensionEndToEndTest {
  ...
  void setup(EdcExtension extension) {
    extension.setConfiguration(
        Map.of("web.http.port", String.valueOf(getFreePort()), "web.http.path",
               "/api", "web.http.management.port", String.valueOf(dataPort),
               "web.http.management.path", "/api/v1/management"));

    extension.registerServiceMock(TransferWaitStrategy.class, () -> 1);
    extension.registerServiceMock(EdcHttpClient.class,
                                  testHttpClient(delegate));
    extension.registerServiceMock(ContractValidationService.class,
    ...
    extension.registerSystemExtension(ServiceExtension.class,
                                      new DummyCallbackUrlExtension());
    extension.setConfiguration(PROVISIONER_CONFIG);
    extension.registerSystemExtension(
        ServiceExtension.class, new ServiceExtension() {
          @Inject
          private AssetIndex
              assetIndex; // needed for on-demand dependency resolution
        });
  }

  /**
   * Tests the case where an initial request returns a retryable failure and the
   * second request completes.
   */
  @Test
  void
  processProviderRequestRetry(TransferProcessProtocolService protocolService,
                              ContractNegotiationStore negotiationStore,
                              AssetIndex assetIndex, TransferProcessStore store,
                              PolicyDefinitionStore policyStore)
      throws Exception {
    when(contractValidationService.validateAgreement(any(), any()))
    ...
    var result = protocolService.notifyRequested(
        createTransferRequestMessage(),
    ...
    await().untilAsserted(() -> {
      ...
      assertThat(transferProcess)
          ...
          .isEqualTo(PROVISIONING_REQUESTED.code());
    });
  }
  ...
  private Asset createAssetEntry() {
    return Asset.Builder.newInstance()
        .id(ASSET_ID)
        .dataAddress(
            DataAddress.Builder.newInstance().type(TEST_DATA_TYPE).build())
        .build();
  }
