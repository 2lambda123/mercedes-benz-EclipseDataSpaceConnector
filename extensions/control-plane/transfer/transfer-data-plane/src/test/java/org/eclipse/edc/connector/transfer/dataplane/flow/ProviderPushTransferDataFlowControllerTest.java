...
class ProviderPushTransferDataFlowControllerTest {
  ...
  private final ProviderPushTransferDataFlowController flowController =
      new ProviderPushTransferDataFlowController(
          ()
              -> URI.create("http://localhost"),
          selectorService, dataPlaneClientFactory);

  @Test
  void canHandle() {
    assertThat(flowController.canHandle(transferProcess(HTTP_PROXY))).isFalse();
    assertThat(flowController.canHandle(transferProcess("not-http-proxy")))
        .isTrue();
  }
  ...
  void initiateFlow_transferSuccess() {
    ...
    when(dataPlaneClient.transfer(any(DataFlowRequest.class)))
    ...
    when(dataPlaneClientFactory.createClient(any()))
        .thenReturn(dataPlaneClient);

    var result = flowController.initiateFlow(
        transferProcess, Policy.Builder.newInstance().build());
    ...
    assertThat(captured.getSourceDataAddress())
        .usingRecursiveComparison()
        .isEqualTo(source);
    assertThat(captured.getDestinationDataAddress())
        .usingRecursiveComparison()
        .isEqualTo(request.getDataDestination());
  ...
  void initiateFlow_returnFailedResultIfTransferFails() {
    ...
    when(dataPlaneClient.transfer(any()))
        .thenReturn(StatusResult.failure(ResponseStatus.FATAL_ERROR, errorMsg));
    var dataPlaneInstance = createDataPlaneInstance();
    when(selectorService.select(any(), any())).thenReturn(dataPlaneInstance);
    when(dataPlaneClientFactory.createClient(any()))
        .thenReturn(dataPlaneClient);
  ...
  void transferTypes_shouldReturnTypesForSpecifiedAsset() {
    when(selectorService.getAll())
        .thenReturn(List.of(dataPlaneInstanceBuilder()
                                .allowedSourceType("TargetSrc")
                                .allowedDestType("TargetDest")
                                .build(),
                            dataPlaneInstanceBuilder()
                                .allowedSourceType("TargetSrc")
                                .allowedDestType("AnotherTargetDest")
                                .build(),
                            dataPlaneInstanceBuilder()
                                .allowedSourceType("AnotherSrc")
                                .allowedDestType("ThisWontBeListed")
                                .build()));
    var asset =
        Asset.Builder.newInstance()
            .dataAddress(
                DataAddress.Builder.newInstance().type("TargetSrc").build())
            .build();

    var transferTypes = flowController.transferTypesFor(asset);

    assertThat(transferTypes)
        .containsExactly("TargetDest-PUSH", "AnotherTargetDest-PUSH");
  }
  ...
  private DataRequest createDataRequest() { return createDataRequest("test"); }
