...
class ContractNegotiationServiceImplTest {
  ...
  private final ContractNegotiationService service =
      new ContractNegotiationServiceImpl(
          store, consumerManager, transactionContext, commandHandlerRegistry);
  ...
  void search_filtersBySpec() {
    var negotiation = createContractNegotiation("negotiationId");
    when(store.queryNegotiations(isA(QuerySpec.class)))
        .thenReturn(Stream.of(negotiation));
    ...
    assertThat(result.getContent())
        .hasSize(1)
        .first()
        .matches(it -> it.getId().equals("negotiationId"));
  }

  @ParameterizedTest
  @ArgumentsSource(InvalidFilters.class)
  void search_invalidFilter(Criterion invalidFilter) {
    var query = QuerySpec.Builder.newInstance().filter(invalidFilter).build();
  ...
  void search_validFilter(Criterion validFilter) {
    var query = QuerySpec.Builder.newInstance().filter(validFilter).build();
  ...
  void initiateNegotiation_callsManager() {
    ...
    when(consumerManager.initiate(isA(ContractRequest.class)))
        .thenReturn(StatusResult.success(contractNegotiation));
  ...
  private static class InvalidFilters implements ArgumentsProvider {
    ...
    provideArguments(ExtensionContext context) {
      return Stream.of(
          arguments(criterion("contractAgreement.contractStartDate.begin", "=",
                              "123455")), // invalid path
          arguments(criterion("contractOffers.policy.unexistent", "=",
                              "123455")), // invalid path
          arguments(criterion("contractOffers.policy.assetid", "=",
                              "123455")), // wrong case
          arguments(criterion("contractOffers.policy.=some-id", "=",
                              "123455")) // incomplete path
      );
    }
  }
  ...
  private static class ValidFilters implements ArgumentsProvider {
    ...
    provideArguments(ExtensionContext context) {
      return Stream.of(
          ...
          arguments(
              criterion("contractAgreement.policy.assignee", "=", "123455")));
    }
  }
