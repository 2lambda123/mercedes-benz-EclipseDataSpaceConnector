...
class CatalogProtocolServiceImplTest {
  ...
  private final CatalogProtocolServiceImpl service =
      new CatalogProtocolServiceImpl(
          datasetResolver, participantAgentService, dataServiceRegistry,
          identityService, mock(), "participantId", transactionContext);
  ...
  void getCatalog_shouldReturnCatalogWithConnectorDataServiceAndItsDataset() {
    ...
    when(datasetResolver.query(any(), any()))
        .thenReturn(Stream.of(createDataset()));
    ...
    assertThat(result).isSucceeded().satisfies(catalog -> {
      assertThat(catalog.getDataServices())
          .hasSize(1)
          .first()
          .isSameAs(dataService);
