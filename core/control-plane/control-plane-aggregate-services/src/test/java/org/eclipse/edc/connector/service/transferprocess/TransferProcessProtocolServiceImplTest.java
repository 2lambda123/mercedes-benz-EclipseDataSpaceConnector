...
class TransferProcessProtocolServiceImplTest {
  ...
  void setUp() {
    ...
    service = new TransferProcessProtocolServiceImpl(
        store, transactionContext, negotiationStore, validationService,
        identityService, dataAddressValidator, observable, mock(), mock(),
        mock());
  }
  ...
  void notifyRequested_validAgreement_shouldInitiateTransfer() {
    ...
    var message = TransferRequestMessage.Builder.newInstance()
                      ...
                      .dataDestination(
                          DataAddress.Builder.newInstance().type("any").build())
                      .build();
    ...
    when(negotiationStore.findContractAgreement(any()))
        .thenReturn(contractAgreement());
    when(validationService.validateAgreement(any(), any()))
        .thenReturn(Result.success(null));
    when(dataAddressValidator.validateDestination(any()))
  ...
  void notifyRequested_doNothingIfProcessAlreadyExist() {
    ...
    when(dataAddressValidator.validateDestination(any()))
        .thenReturn(ValidationResult.success());
    when(store.findForCorrelationId(any()))
        .thenReturn(transferProcess(REQUESTED, "transferProcessId"));
  ...
  void notifyRequested_invalidDestination_shouldNotInitiateTransfer() {
    ...
    when(dataAddressValidator.validateDestination(any()))
        .thenReturn(ValidationResult.failure(
            violation("invalid data address", "path")));
  ...
  void notifyStarted_shouldTransitionToStarted() {
    ...
    when(identityService.verifyJwtToken(eq(tokenRepresentation),
                                        isA(VerificationContext.class)))
        .thenReturn(Result.success(claimToken));
    when(store.findByCorrelationIdAndLease("correlationId"))
        .thenReturn(StoreResult.success(
            transferProcess(REQUESTED, "transferProcessId")));
    when(negotiationStore.findContractAgreement(any())).thenReturn(agreement);
    when(validationService.validateRequest(claimToken, agreement))
    ...
    assertThat(captor.getValue().getDataAddress())
        .usingRecursiveComparison()
        .isEqualTo(message.getDataAddress());
  }
  ...
  void notifyStarted_shouldReturnConflict_whenStatusIsNotValid() {
    ...
    var tokenRepresentation = tokenRepresentation();
    var transferProcess =
        transferProcess(COMPLETED, UUID.randomUUID().toString());
    ...
    when(identityService.verifyJwtToken(eq(tokenRepresentation),
        ...
        .thenReturn(Result.success(claimToken));
    when(store.findByCorrelationIdAndLease("correlationId"))
        .thenReturn(StoreResult.success(transferProcess));
    when(negotiationStore.findContractAgreement(any())).thenReturn(agreement);
    when(validationService.validateRequest(claimToken, agreement))
        .thenReturn(Result.success());
    ...
    verify(store, times(1))
        .save(argThat(tp -> tp.getState() == COMPLETED.code()));
    verifyNoInteractions(listener);
  }
  ...
  void notifyCompleted_shouldTransitionToCompleted() {
    ...
    when(store.findByCorrelationIdAndLease("correlationId"))
        .thenReturn(
            StoreResult.success(transferProcess(STARTED, "transferProcessId")));
    when(negotiationStore.findContractAgreement(any())).thenReturn(agreement);
    when(validationService.validateRequest(claimToken, agreement))
        .thenReturn(Result.success());
  ...
  void notifyCompleted_shouldReturnConflict_whenStatusIsNotValid() {
    ...
    var tokenRepresentation = tokenRepresentation();
    var transferProcess =
        transferProcess(REQUESTED, UUID.randomUUID().toString());
    var message = TransferCompletionMessage.Builder.newInstance()
                      .protocol("protocol")
                      .counterPartyAddress("http://any")
                      .processId("correlationId")
    ...
    // state didn't change
    verify(store, times(1))
        .save(argThat(tp -> tp.getState() == REQUESTED.code()));
    verifyNoInteractions(listener);
  }
  ...
  void notifyTerminated_shouldReturnConflict_whenStatusIsNotValid() {
    ...
    var tokenRepresentation = tokenRepresentation();
    var transferProcess =
        transferProcess(TERMINATED, UUID.randomUUID().toString());
    var agreement = contractAgreement();
    var message = TransferTerminationMessage.Builder.newInstance()
                      .protocol("protocol")
                      .counterPartyAddress("http://any")
    ...
    // state didn't change
    verify(store, times(1))
        .save(argThat(tp -> tp.getState() == TERMINATED.code()));
    verifyNoInteractions(listener);
  }
  ...
  notify_shouldFail_whenTransferProcessNotFound(MethodCall<M> methodCall,
                                                M message) {
    var claimToken = claimToken();
    var tokenRepresentation = tokenRepresentation();
  ...
  @ArgumentsSource(NotFoundArguments.class)
  <M extends RemoteMessage> void
  notify_shouldFail_whenTokenValidationFails(MethodCall<M> methodCall,
                                             M message) {
    var tokenRepresentation = tokenRepresentation();
  ...
  private interface MethodCall<M extends RemoteMessage> {
    ServiceResult<?> call(TransferProcessProtocolService service, M message,
  ...
  private static class NotFoundArguments implements ArgumentsProvider {
    ...
    provideArguments(ExtensionContext extensionContext) {
      MethodCall<TransferStartMessage> started =
          TransferProcessProtocolService::notifyStarted;
      MethodCall<TransferCompletionMessage> completed =
          TransferProcessProtocolService::notifyCompleted;
      MethodCall<TransferTerminationMessage> terminated =
          TransferProcessProtocolService::notifyTerminated;
      return Stream.of(
          Arguments.of(started, TransferStartMessage.Builder.newInstance()
                                    .protocol("protocol")
                                    .counterPartyAddress("http://any")
                                    .processId("correlationId")
                                    .build()),
