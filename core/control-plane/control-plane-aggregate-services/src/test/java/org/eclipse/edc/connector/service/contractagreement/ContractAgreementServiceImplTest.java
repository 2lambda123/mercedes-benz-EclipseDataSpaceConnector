...
class ContractAgreementServiceImplTest {
  ...
  private final ContractAgreementService service =
      new ContractAgreementServiceImpl(store, transactionContext);
  ...
  void search_filtersBySpec() {
    var agreement = createContractAgreement("agreementId");
    when(store.queryAgreements(isA(QuerySpec.class)))
        .thenReturn(Stream.of(agreement));
    ...
    assertThat(result.getContent())
        .hasSize(1)
        .first()
        .matches(it -> it.getId().equals("agreementId"));
  }
  ...
  void findNegotiation_shouldReturnNegotiationFilteredByAgreementId() {
    ...
    var expectedCriterion =
        criterion("contractAgreement.id", "=", "agreementId");
    verify(store).queryNegotiations(
        argThat(q -> q.getFilterExpression().contains(expectedCriterion)));
  }

  @Test
  void
  findNegotiation_shouldReturnNull_whenAgreementOrNegotiationDoesNotExist() {
