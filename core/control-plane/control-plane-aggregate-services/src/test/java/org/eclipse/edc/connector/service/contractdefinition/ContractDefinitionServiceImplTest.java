...
class ContractDefinitionServiceImplTest {
  ...
  private final ContractDefinitionObservable observable =
      new ContractDefinitionObservableImpl();
  private final ContractDefinitionListener listener = mock();

  private final ContractDefinitionService service =
      new ContractDefinitionServiceImpl(store, transactionContext, observable);
  ...
  void search() {
    ...
    assertThat(result.getContent())
        .hasSize(1)
        .first()
        .matches(hasId(definition.getId()));
  }
  ...
  void create_shouldNotCreateDefinitionIfTheStoreFails() {
    ...
    when(store.save(definition))
        .thenReturn(StoreResult.alreadyExists("Exists"));
  ...
  void delete_shouldDeleteDefinitionIfItsNotReferencedByAnyAgreement() {
    var definition = createContractDefinition();
    when(store.deleteById("assetId"))
        .thenReturn(StoreResult.success(definition));
  ...
  private static class InvalidFilters implements ArgumentsProvider {
    ...
    provideArguments(ExtensionContext context) {
      return Stream.of(
          arguments(
              criterion("assetsSelector.leftHand", "=", "foo")), // invalid path
          arguments(criterion(
              "accessPolicyId'LIKE/**/?/**/LIMIT/**/?/**/OFFSET/**/?;DROP/**/TABLE/**/test/**/--%20",
              "=", "%20ABC--")) // some SQL injection
      );
    }
  }
