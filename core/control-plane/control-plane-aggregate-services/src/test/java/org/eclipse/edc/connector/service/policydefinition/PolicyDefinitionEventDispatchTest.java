...
public class PolicyDefinitionEventDispatchTest {
  ...
  @Test
  void shouldDispatchEventOnPolicyDefinitionCreationAndDeletionAndUpdate(
      PolicyDefinitionService service, EventRouter eventRouter) {
    ...
    await().untilAsserted(() -> {
      verify(eventSubscriber)
          .on(argThat(isEnvelopeOf(PolicyDefinitionCreated.class)));
    });

    service.update(policyDefinition);
    await().untilAsserted(() -> {
      verify(eventSubscriber)
          .on(argThat(isEnvelopeOf(PolicyDefinitionUpdated.class)));
    });

    service.deleteById(policyDefinition.getUid());
    await().untilAsserted(() -> {
      verify(eventSubscriber)
          .on(argThat(isEnvelopeOf(PolicyDefinitionDeleted.class)));
    });
  }
}
