...
class ContractNegotiationProtocolServiceImplTest {
  ...
  private final TransactionContext transactionContext =
      spy(new NoopTransactionContext());
  ...
  void setUp() {
    ...
    service = new ContractNegotiationProtocolServiceImpl(
        store, transactionContext, validationService, identityService,
  ...
  void notifyRequested_shouldInitiateNegotiation_whenNegotiationDoesNotExist() {
    ...
    assertThat(calls.getAllValues()).anySatisfy(n -> {
      ...
      assertThat(n.getCounterPartyAddress())
          .isEqualTo(message.getCallbackAddress());
    ...
    verify(transactionContext, atLeastOnce())
        .execute(any(TransactionContext.ResultTransactionBlock.class));
  }
  ...
  void notifyRequested_shouldTransitionToRequested_whenNegotiationFound() {
    ...
    when(identityService.verifyJwtToken(eq(tokenRepresentation),
        ...
        .thenReturn(Result.success(claimToken));
    when(store.findByCorrelationIdAndLease(any()))
        .thenReturn(StoreResult.success(negotiation));
    when(validationService.validateInitialOffer(claimToken, contractOffer))
        .thenReturn(Result.success(validatedOffer));
  ...
  void notifyOffered_shouldTransitionToOffered_whenNegotiationFound() {
    ...
    when(identityService.verifyJwtToken(eq(tokenRepresentation),
                                        isA(VerificationContext.class)))
        .thenReturn(Result.success(claimToken));
    when(store.findByCorrelationIdAndLease(processId))
        .thenReturn(StoreResult.success(negotiation));
    when(validationService.validateRequest(claimToken, negotiation))
    ...
    assertThat(updatedNegotiation.getLastContractOffer())
        .isEqualTo(contractOffer);
  ...
  void notifyAccepted_shouldTransitionToAccepted() {
    ...
    when(identityService.verifyJwtToken(eq(tokenRepresentation),
        ...
        .thenReturn(Result.success(claimToken));
    when(store.findByCorrelationIdAndLease("processId"))
        .thenReturn(StoreResult.success(contractNegotiation));
    when(validationService.validateRequest(eq(claimToken),
    ...
    assertThat(result).isSucceeded();
    verify(store).save(
        argThat(negotiation -> negotiation.getState() == ACCEPTED.code()));
  ...
  void notifyAgreed_shouldTransitionToAgreed() {
    ...
    when(identityService.verifyJwtToken(eq(tokenRepresentation),
        ...
        .thenReturn(Result.success(claimToken));
    when(store.findByCorrelationIdAndLease("processId"))
        .thenReturn(StoreResult.success(negotiationConsumerRequested));
    when(validationService.validateConfirmed(
             eq(claimToken), eq(contractAgreement), any(ContractOffer.class)))
        .thenReturn(Result.success());
    ...
    verify(validationService)
        .validateConfirmed(eq(claimToken), eq(contractAgreement),
  ...
  void notifyVerified_shouldTransitionToVerified() {
    ...
    verify(validationService)
        .validateRequest(any(), any(ContractNegotiation.class));
    verify(transactionContext, atLeastOnce())
        .execute(any(TransactionContext.ResultTransactionBlock.class));
  }
  ...
  void findById_shouldReturnNegotiation_whenValidCounterParty() {
    ...
    assertThat(result).isSucceeded().isEqualTo(negotiation);
  }
  ...
  void findById_shouldReturnBadRequest_whenCounterPartyUnauthorized() {
    ...
    when(store.findById(id)).thenReturn(negotiation);
    when(validationService.validateRequest(claimToken, negotiation))
        .thenReturn(Result.failure("validation error"));
  ...
  <M extends RemoteMessage> void
  notify_shouldReturnNotFound_whenNotFound(MethodCall<M> methodCall,
                                           M message) {
    var claimToken = claimToken();
    var tokenRepresentation = tokenRepresentation();
    when(identityService.verifyJwtToken(eq(tokenRepresentation),
    ...
    // currently ContractRequestMessage cannot happen on an already existing
    // negotiation
  ...
  @ArgumentsSource(NotifyArguments.class)
  <M extends RemoteMessage> void
  notify_shouldReturnBadRequest_whenValidationFails(MethodCall<M> methodCall,
                                                    M message) {
    var claimToken = claimToken();
    var tokenRepresentation = tokenRepresentation();
    ...
    when(identityService.verifyJwtToken(eq(tokenRepresentation),
        ...
        .thenReturn(Result.success(claimToken));
    when(store.findByCorrelationIdAndLease(any()))
        .thenReturn(StoreResult.success(createContractNegotiationOffered()));
    when(validationService.validateRequest(any(),
                                           any(ContractNegotiation.class)))
        .thenReturn(Result.failure("validation error"));
    when(
        validationService.validateInitialOffer(any(), any(ContractOffer.class)))
        .thenReturn(Result.failure("error"));
    when(validationService.validateConfirmed(any(), any(),
                                             any(ContractOffer.class)))
  ...
  @ArgumentsSource(NotifyArguments.class)
  <M extends RemoteMessage> void
  notify_shouldReturnBadRequest_whenTokenValidationFails(
      MethodCall<M> methodCall, M message) {
    var tokenRepresentation = tokenRepresentation();

    when(identityService.verifyJwtToken(eq(tokenRepresentation),
                                        isA(VerificationContext.class)))
        .thenReturn(Result.failure("unauthorized"));
  ...
  private interface MethodCall<M extends RemoteMessage> {
    ServiceResult<?> call(ContractNegotiationProtocolService service, M message,
  ...
  private static class NotifyArguments implements ArgumentsProvider {
    ...
    provideArguments(ExtensionContext extensionContext) {
      MethodCall<ContractRequestMessage> requested =
          ContractNegotiationProtocolService::notifyRequested;
      MethodCall<ContractOfferMessage> offered =
          ContractNegotiationProtocolService::notifyOffered;
      MethodCall<ContractAgreementMessage> agreed =
          ContractNegotiationProtocolService::notifyAgreed;
      MethodCall<ContractNegotiationEventMessage> accepted =
          ContractNegotiationProtocolService::notifyAccepted;
      MethodCall<ContractAgreementVerificationMessage> verified =
          ContractNegotiationProtocolService::notifyVerified;
      MethodCall<ContractNegotiationEventMessage> finalized =
          ContractNegotiationProtocolService::notifyFinalized;
      MethodCall<ContractNegotiationTerminationMessage> terminated =
          ContractNegotiationProtocolService::notifyTerminated;
      return Stream.of(
          ...
          Arguments.of(accepted,
                       ContractNegotiationEventMessage.Builder.newInstance()
                           ...
                           .processId("processId")
                           .build()),
          Arguments.of(verified, ContractAgreementVerificationMessage.Builder
                                     .newInstance()
                                     .protocol("protocol")
                                     .counterPartyAddress("http://any")
                                     .processId("processId")
                                     .build()),
          Arguments.of(finalized,
                       ContractNegotiationEventMessage.Builder.newInstance()
                           .type(ContractNegotiationEventMessage.Type.FINALIZED)
                           .protocol("protocol")
                           .counterPartyAddress("http://any")
                           .processId("processId")
                           .build()),
          Arguments.of(terminated, ContractNegotiationTerminationMessage.Builder
                                       .newInstance()
                                       .protocol("protocol")
                                       .processId("processId")
                                       .counterPartyAddress("http://any")
