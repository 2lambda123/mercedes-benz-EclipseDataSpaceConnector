...
class PolicyDefinitionServiceImplTest {
  ...
  private final PolicyDefinitionObservable observable =
      mock(PolicyDefinitionObservable.class);
  private final PolicyDefinitionServiceImpl policyServiceImpl =
      new PolicyDefinitionServiceImpl(dummyTransactionContext, policyStore,
                                      contractDefinitionStore, observable);
  ...
  void search_shouldRelyOnPolicyStore() {
    ...
    when(policyStore.findAll(any(QuerySpec.class)))
        .thenReturn(Stream.of(policy));
  ...
  void createPolicy_shouldNotCreatePolicyIfItAlreadyExists() {
    var policy = createPolicy("policyId");
    when(policyStore.create(policy))
  ...
  void delete_shouldDeletePolicyIfItsNotReferencedByAnyContractDefinition() {
    when(contractDefinitionStore.findAll(any()))
        .thenReturn(Stream.empty(), Stream.empty());
    when(policyStore.findById(any())).thenReturn(createPolicy("policyId"));
    when(policyStore.delete("policyId"))
        .thenReturn(StoreResult.success(createPolicy("policyId")));
  ...
  void delete_shouldNotDelete_whenPolicyPartOfContractDef() {
    var policy = createPolicy("policyId");
    when(policyStore.delete("policyId"))
        .thenReturn(StoreResult.success(policy));
  ...
  private static class InvalidFilters implements ArgumentsProvider {
    ...
    provideArguments(ExtensionContext context) {
      return Stream.of(
          arguments(criterion("policy.permissions.action.constraint.noexist",
                              "=", "123455")), // wrong property
          arguments(criterion("permissions.action.constraint.leftExpression",
                              "=", "123455")), // missing root
          arguments(criterion("policy.permissions.action.leftExpression", "=",
                              "123455")) // skips path element
      );
    }
  }
