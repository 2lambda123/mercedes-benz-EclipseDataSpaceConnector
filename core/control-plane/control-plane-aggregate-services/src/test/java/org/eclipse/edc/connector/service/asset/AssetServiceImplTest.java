/*
 *  Copyright (c) 2021 - 2022 Bayerische Motoren Werke Aktiengesellschaft (BMW
 * AG)
 *
...
class AssetServiceImplTest {
  ...
  private final TransactionContext dummyTransactionContext =
  ...
  private final AssetService service = new AssetServiceImpl(
      index, contractNegotiationStore, dummyTransactionContext, observable,
  ...
  void search_shouldRelyOnAssetIndex() {
    ...
    assertThat(assets.getContent())
        .hasSize(1)
        .first()
        .matches(hasId("assetId"));
  }

  @ParameterizedTest
  @ValueSource(strings = {Asset.PROPERTY_ID, Asset.PROPERTY_NAME,
                          Asset.PROPERTY_DESCRIPTION, Asset.PROPERTY_VERSION,
                          Asset.PROPERTY_CONTENT_TYPE})
  void
  search_validFilter(String filter) {
  ...
  void createAsset_shouldCreateAssetIfItDoesNotAlreadyExist() {
    when(dataAddressValidator.validateSource(any()))
    ...
    verify(index).create(
        and(isA(Asset.class), argThat(it -> assetId.equals(it.getId()))));
  ...
  void createAsset_shouldNotCreateAssetIfDataAddressInvalid() {
    ...
    when(dataAddressValidator.validateSource(any()))
        .thenReturn(ValidationResult.failure(
            violation("Data address is invalid", "path")));
  ...
  void delete_shouldDeleteAssetIfItsNotReferencedByAnyNegotiation() {
    when(contractNegotiationStore.queryNegotiations(any()))
        .thenReturn(Stream.empty());
    when(index.deleteById("assetId"))
        .thenReturn(StoreResult.success(createAsset("assetId")));
  ...
  void delete_shouldNotDeleteIfAssetIsAlreadyPartOfAnAgreement() {
    ...
    var contractNegotiation =
        ContractNegotiation.Builder.newInstance()
            ...
            .build();
    when(contractNegotiationStore.queryNegotiations(any()))
        .thenReturn(Stream.of(contractNegotiation));
  ...
  void delete_verifyCorrectQuery() {
    ...
    assertThat(deleted.succeeded()).isTrue();
    verify(contractNegotiationStore)
        .queryNegotiations(
            argThat(argument
                    -> argument.getFilterExpression().size() == 1 &&
                           argument.getFilterExpression()
                               .get(0)
                               .getOperandLeft()
                               .equals("contractAgreement.assetId")));
  }
  ...
  private static class InvalidFilters implements ArgumentsProvider {
    ...
    provideArguments(ExtensionContext context) {
      return Stream.of(
          arguments(criterion("  asset_prop_id", "in",
                              "(foo, bar)")), // invalid leading whitespace
          arguments(
              criterion(".customProp", "=", "whatever")) // invalid leading dot
      );
    }
  }
  ...
  private Asset.Builder createAssetBuilder(String assetId) {
    return Asset.Builder.newInstance().id(assetId).dataAddress(
