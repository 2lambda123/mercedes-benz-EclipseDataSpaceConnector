...
class TransferProcessServiceImplTest {
  ...
  private final QuerySpec query =
      QuerySpec.Builder.newInstance().limit(5).offset(2).build();
  ...
  private final TransferProcessService service = new TransferProcessServiceImpl(
      store, manager, transactionContext, dataAddressValidator,
  ...
  void getState_whenFound() {
    when(store.findById(id)).thenReturn(process1);
    assertThat(service.getState(id))
        .isEqualTo(TransferProcessStates.from(process1.getState()).name());
    verify(transactionContext)
        .execute(any(TransactionContext.ResultTransactionBlock.class));
  }
  ...
  void initiateTransfer() {
    ...
    when(manager.initiateConsumerRequest(transferRequest))
        .thenReturn(StatusResult.success(transferProcess));
  ...
  @Test
  void
  initiateTransfer_consumer_invalidDestination_shouldNotInitiateTransfer() {
    when(dataAddressValidator.validateDestination(any()))
    ...
    Assertions.assertThat(result)
        .satisfies(ServiceResult::failed)
  ...
  void terminate_shouldExecuteCommandAndReturnResult() {
    when(commandHandlerRegistry.execute(any()))
  ...
  void terminate_shouldFailWhenCommandHandlerFails() {
    when(commandHandlerRegistry.execute(any()))
        .thenReturn(CommandResult.notFound("not found"));
    var command = new TerminateTransferCommand("id", "reason");
  ...
  private static class InvalidFilters implements ArgumentsProvider {
    ...
    provideArguments(ExtensionContext context) {
      return Stream.of(
          arguments(criterion("provisionedResourceSet.resources.hastoken", "=",
                              "true")), // wrong case
          arguments(criterion("resourceManifest.definitions.notexist", "=",
                              "foobar")), // property not exist
          arguments(criterion("contentDataAddress.properties.someKey", "=",
                              "someval")) // map types not supported
      );
    }
  }
  ...
  private static class ValidFilters implements ArgumentsProvider {
    ...
    provideArguments(ExtensionContext context) {
      return Stream.of(
          arguments(criterion("deprovisionedResources.provisionedResourceId",
                              "=", "someval")),
  ...
  private TransferProcess transferProcess() {
    var state =
        TransferProcessStates.values()[ThreadLocalRandom.current().nextInt(
            TransferProcessStates.values().length)];
