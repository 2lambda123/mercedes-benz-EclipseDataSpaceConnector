 ...
 *       Fraunhofer Institute for Software and Systems Engineering - initiate
 * provider process
 *
 */
...
public class TransferProcessProtocolServiceImpl
    extends BaseProtocolService implements TransferProcessProtocolService {
  ...
  public TransferProcessProtocolServiceImpl(
      ...
      TransferProcessObservable observable, Clock clock, Monitor monitor,
      Telemetry telemetry) {
  ...
  @WithSpan
  @NotNull
  public ServiceResult<TransferProcess>
  notifyRequested(TransferRequestMessage message,
    ...
    return verifyToken(tokenRepresentation).compose(claimToken -> {
      ...
      if (destination != null) {
        var validDestination =
            dataAddressValidator.validateDestination(destination);
        if (validDestination.failed()) {
          return ServiceResult.badRequest(
              validDestination.getFailureMessages());
        }
      }
      ...
      return transactionContext.execute(
          ()
              -> Optional
                     .ofNullable(negotiationStore.findContractAgreement(
                     ...
                     .filter(agreement
                             -> contractValidationService
                                    ...
                                    .succeeded())
                     .map(agreement
                          -> requestedAction(message, agreement.getAssetId()))
                     .orElse(ServiceResult.conflict(
                         format("Cannot process %s because %s",
                                message.getClass().getSimpleName(),
                                "agreement not found or not valid"))));
    });
  }
  ...
  @NotNull
  public ServiceResult<TransferProcess>
  notifyStarted(TransferStartMessage message,
                TokenRepresentation tokenRepresentation) {
    return verifyToken(tokenRepresentation)
        .compose(
            ...
            -> onMessageDo(
                message, claimToken,
                transferProcess -> startedAction(message, transferProcess)));
  }
  ...
  @NotNull
  public ServiceResult<TransferProcess>
  notifyCompleted(TransferCompletionMessage message,
                  TokenRepresentation tokenRepresentation) {
    return verifyToken(tokenRepresentation)
        .compose(
            claimToken
            -> onMessageDo(
                message, claimToken,
                transferProcess -> completedAction(message, transferProcess)));
  }
  ...
  @NotNull
  public ServiceResult<TransferProcess>
  notifyTerminated(TransferTerminationMessage message,
                   TokenRepresentation tokenRepresentation) {
    return verifyToken(tokenRepresentation)
        .compose(
            claimToken
            -> onMessageDo(
                message, claimToken,
                transferProcess -> terminatedAction(message, transferProcess)));
  }
    ...
    return verifyToken(tokenRepresentation)
        .compose(
            ...
            -> transactionContext.execute(
                ()
                    -> Optional.ofNullable(transferProcessStore.findById(id))
                           .map(tp -> validateCounterParty(claimToken, tp))
                           .orElse(notFound(id))));
  }

  @NotNull
  private ServiceResult<TransferProcess>
  requestedAction(TransferRequestMessage message, String assetId) {
    var destination =
        message.getDataDestination() != null
    ...
    var existingTransferProcess =
        transferProcessStore.findForCorrelationId(dataRequest.getId());
  ...
  @NotNull
  private ServiceResult<TransferProcess>
  startedAction(TransferStartMessage message, TransferProcess transferProcess) {
    if (transferProcess.getType() == CONSUMER &&
        transferProcess.canBeStartedConsumer()) {
      ...
      observable.invokeForEach(
          l -> l.started(transferProcess, transferStartedData));
      return ServiceResult.success(transferProcess);
    } else {
      return ServiceResult.conflict(format("Cannot process %s because %s",
                                           message.getClass().getSimpleName(),
  ...
  @NotNull
  private ServiceResult<TransferProcess>
  completedAction(TransferCompletionMessage message,
  ...
  @NotNull
  private ServiceResult<TransferProcess>
  terminatedAction(TransferTerminationMessage message,
                   TransferProcess transferProcess) {
    if (transferProcess.canBeTerminated()) {
      ...
      return ServiceResult.conflict(format("Cannot process %s because %s",
                                           message.getClass().getSimpleName(),
                                           "transfer cannot be terminated"));
    }
  }

  private ServiceResult<TransferProcess> onMessageDo(
      TransferRemoteMessage message, ClaimToken claimToken,
    ...
    return transactionContext.execute(
        ()
            -> transferProcessStore
                   ...
                   .compose(
                       transferProcess
                       -> validateCounterParty(claimToken, transferProcess)
                              .compose(action)
                              .onFailure(f -> breakLease(transferProcess))));
  }

  private ServiceResult<TransferProcess>
  validateCounterParty(ClaimToken claimToken, TransferProcess transferProcess) {
    return Optional
        .ofNullable(negotiationStore.findContractAgreement(
            transferProcess.getContractId()))
        .map(
            agreement
            -> contractValidationService.validateRequest(claimToken, agreement))
  ...
  private ServiceResult<TransferProcess> notFound(String transferProcessId) {
    return ServiceResult.notFound(
        format("No transfer process with id %s found", transferProcessId));
  }
  ...
  private void update(TransferProcess transferProcess) {
    ...
    monitor.debug(
        format("TransferProcess %s is now in state %s", transferProcess.getId(),
