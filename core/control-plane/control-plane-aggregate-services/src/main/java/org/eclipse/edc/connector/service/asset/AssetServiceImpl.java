...
public class AssetServiceImpl implements AssetService {
  ...
  private static final String DUPLICATED_KEYS_MESSAGE =
      "Duplicate keys in properties and private properties are not allowed";
  ...
  public AssetServiceImpl(AssetIndex index,
                          ContractNegotiationStore contractNegotiationStore,
                          TransactionContext transactionContext,
                          AssetObservable observable,
  ...
  public ServiceResult<Asset> create(Asset asset) {
    ...
    var validDataAddress =
        dataAddressValidator.validateSource(asset.getDataAddress());
  ...
  public ServiceResult<Asset> delete(String assetId) {
    return transactionContext.execute(() -> {
      ...
      try (var negotiationsOnAsset =
               contractNegotiationStore.queryNegotiations(query)) {
        if (negotiationsOnAsset.findAny().isPresent()) {
          return ServiceResult.conflict(format(
              "Asset %s cannot be deleted as it is referenced by at least one contract agreement",
              assetId));
        }
      }
