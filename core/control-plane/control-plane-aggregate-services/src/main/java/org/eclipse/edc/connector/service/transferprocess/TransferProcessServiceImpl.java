...
public class TransferProcessServiceImpl implements TransferProcessService {
  ...
  public TransferProcessServiceImpl(
      TransferProcessStore transferProcessStore, TransferProcessManager manager,
  ...
  public @Nullable TransferProcess findById(String transferProcessId) {
    return transactionContext.execute(
        () -> transferProcessStore.findById(transferProcessId));
  }
  ...
  public @Nullable String getState(String transferProcessId) {
    return transactionContext.execute(() -> {
      ...
      return Optional.ofNullable(process)
          .map(p -> TransferProcessStates.from(p.getState()).name())
          .orElse(null);
    });
  }
  ...
  @Override
  public @NotNull ServiceResult<Void>
  terminate(TerminateTransferCommand command) {
    return transactionContext.execute(
        ()
            -> commandHandlerRegistry.execute(command).flatMap(
                ServiceResult::from));
  ...
  @Override
  public @NotNull ServiceResult<TransferProcess>
  initiateTransfer(TransferRequest request) {
    var validDestination =
        dataAddressValidator.validateDestination(request.getDataDestination());
  ...
  @Override
  public ServiceResult<Void>
  addProvisionedResource(String transferProcessId, ProvisionResponse response) {
    var command =
        new AddProvisionedResourceCommand(transferProcessId, response);
    return transactionContext.execute(
        ()
            -> commandHandlerRegistry.execute(command).flatMap(
                ServiceResult::from));
  ...
  private Map<Class<?>, List<Class<?>>> getSubtypes() {
    return Map.of(ProvisionedResource.class,
                  List.of(ProvisionedDataAddressResource.class),
                  ProvisionedDataAddressResource.class,
                  List.of(ProvisionedDataDestinationResource.class,
                          ProvisionedContentResource.class));
  }
}
