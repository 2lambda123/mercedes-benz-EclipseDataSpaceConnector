...
public class DataFlowManagerImpl implements DataFlowManager {
  ...
  private final List<PrioritizedDataFlowController> controllers =
      new ArrayList<>();
  ...
  initiate(TransferProcess transferProcess, Policy policy) {
    try {
      return chooseControllerAndApply(
          transferProcess,
          controller -> controller.initiateFlow(transferProcess, policy));
    } catch (Exception e) {
      return StatusResult.failure(
          FATAL_ERROR,
          runtimeException(transferProcess.getId(), e.getLocalizedMessage()));
    }
  }

  @Override
  public @NotNull StatusResult<Void>
  terminate(TransferProcess transferProcess) {
    return chooseControllerAndApply(
        transferProcess, controller -> controller.terminate(transferProcess));
  }
  ...
  @NotNull
  private <T> StatusResult<T> chooseControllerAndApply(
      TransferProcess transferProcess,
      Function<DataFlowController, StatusResult<T>> function) {
    return controllers.stream()
        ...
        .orElseGet(
            ()
                -> StatusResult.failure(
                    FATAL_ERROR, controllerNotFound(transferProcess.getId())));
  }

  private String runtimeException(String id, String message) {
    return format(
        "Unable to process transfer %s. Data flow controller throws an exception: %s",
        id, message);
  }

  private String controllerNotFound(String id) {
    return format(
        "Unable to process transfer %s. No data flow controller found", id);
  }

  record PrioritizedDataFlowController(int priority,
                                       DataFlowController controller) {}
}
