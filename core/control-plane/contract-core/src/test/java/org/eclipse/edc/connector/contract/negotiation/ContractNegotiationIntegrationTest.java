/*
 *  Copyright (c) 2021 - 2022 Fraunhofer Institute for Software and Systems
 * Engineering
 *
 ...
 * and implementation Microsoft Corporation - introduced Awaitility
 *
 */
...
class ContractNegotiationIntegrationTest {
  ...
  private final InMemoryContractNegotiationStore providerStore =
      new InMemoryContractNegotiationStore(clock);
  private final InMemoryContractNegotiationStore consumerStore =
      new InMemoryContractNegotiationStore(clock);
  private final ContractValidationService validationService =
      mock(ContractValidationService.class);
  private final RemoteMessageDispatcherRegistry providerDispatcherRegistry =
      mock(RemoteMessageDispatcherRegistry.class);
  private final RemoteMessageDispatcherRegistry consumerDispatcherRegistry =
      mock(RemoteMessageDispatcherRegistry.class);
  private final IdentityService identityService = mock();
  protected ClaimToken token = ClaimToken.Builder.newInstance().build();
  protected TokenRepresentation tokenRepresentation =
  ...
  void init() {
    ...
    providerManager =
        ProviderContractNegotiationManagerImpl.Builder.newInstance()
    ...
    consumerManager =
        ConsumerContractNegotiationManagerImpl.Builder.newInstance()
    ...
    consumerService = new ContractNegotiationProtocolServiceImpl(
        consumerStore, new NoopTransactionContext(), validationService,
        identityService, new ContractNegotiationObservableImpl(), monitor,
        mock(Telemetry.class));
    providerService = new ContractNegotiationProtocolServiceImpl(
        providerStore, new NoopTransactionContext(), validationService,
        identityService, new ContractNegotiationObservableImpl(), monitor,
        mock(Telemetry.class));
  }
  ...
  void testNegotiation_initialOfferAccepted() {
    when(consumerDispatcherRegistry.dispatch(any(),
                                             isA(ContractRequestMessage.class)))
        .then(onConsumerSentOfferRequest());
    when(providerDispatcherRegistry.dispatch(
             any(), isA(ContractAgreementMessage.class)))
        .then(onProviderSentAgreementRequest());
    when(consumerDispatcherRegistry.dispatch(
             any(), isA(ContractAgreementVerificationMessage.class)))
        .then(onConsumerSentAgreementVerification());
    when(providerDispatcherRegistry.dispatch(
             any(), isA(ContractNegotiationEventMessage.class)))
        .then(onProviderSentNegotiationEventMessage());
    consumerNegotiationId = "consumerNegotiationId";
    var offer = getContractOffer();
    when(validationService.validateInitialOffer(token, offer))
        .thenReturn(
            Result.success(new ValidatedConsumerOffer(CONSUMER_ID, offer)));
    when(validationService.validateConfirmed(
             eq(token), any(ContractAgreement.class), any(ContractOffer.class)))
        .thenReturn(Result.success());
    when(validationService.validateRequest(eq(token),
                                           any(ContractNegotiation.class)))
        .thenReturn(Result.success());
    ...
    await()
        .atMost(DEFAULT_TEST_TIMEOUT)
        .pollInterval(DEFAULT_POLL_INTERVAL)
        .untilAsserted(() -> {
          assertThat(consumerNegotiationId).isNotNull();
          var consumerNegotiation =
              consumerStore.findById(consumerNegotiationId);
          var providerNegotiation =
              providerStore.findForCorrelationId(consumerNegotiationId);
          ...
          // Assert that provider and consumer have the same offers and
          // agreement stored
          assertThat(consumerNegotiation.getContractOffers()).hasSize(1);
          assertThat(providerNegotiation.getContractOffers()).hasSize(1);
          assertThat(consumerNegotiation.getContractOffers().get(0))
              .isEqualTo(offer);
          assertThat(consumerNegotiation.getState())
              .isEqualTo(ContractNegotiationStates.FINALIZED.code());
          assertThat(consumerNegotiation.getLastContractOffer())
              .isEqualTo(providerNegotiation.getLastContractOffer());
          assertThat(consumerNegotiation.getContractAgreement())
              .isEqualTo(providerNegotiation.getContractAgreement());

          verify(validationService, atLeastOnce())
              .validateInitialOffer(token, offer);
          verify(validationService, atLeastOnce())
              .validateConfirmed(eq(token), any(ContractAgreement.class),
  ...
  void testNegotiation_initialOfferDeclined() {
    when(providerDispatcherRegistry.dispatch(
             any(), isA(ContractNegotiationTerminationMessage.class)))
        .then(onProviderSentRejection());
    when(consumerDispatcherRegistry.dispatch(any(),
                                             isA(ContractRequestMessage.class)))
        .then(onConsumerSentOfferRequest());
    ...
    when(validationService.validateInitialOffer(token, offer))
        .thenReturn(Result.failure("must be declined"));
  ...
  private Answer<Object> onConsumerSentOfferRequest() {
    return i -> {
      ...
      var result =
          providerService.notifyRequested(request, tokenRepresentation);
  ...
  private Answer<Object> onConsumerSentRejection() {
    return i -> {
      ...
      var result =
          providerService.notifyTerminated(request, tokenRepresentation);
      return toFuture(result);
    };
  }
  ...
  private Answer<Object> onProviderSentNegotiationEventMessage() {
    return i -> {
      ...
      var result =
          consumerService.notifyFinalized(request, tokenRepresentation);
      return toFuture(result);
    };
  }
  ...
  private Answer<Object> onProviderSentRejection() {
    return i -> {
      ContractNegotiationTerminationMessage request = i.getArgument(1);
      var result =
          consumerService.notifyTerminated(request, tokenRepresentation);
      return toFuture(result);
    };
  }
  ...
  toFuture(ServiceResult<ContractNegotiation> result) {
    if (result.succeeded()) {
        ...
        .policy(
            Policy.Builder.newInstance()
                ...
                .duty(Duty.Builder.newInstance()
                          .action(
                              Action.Builder.newInstance().type("USE").build())
                          .build())
                .build())
        .build();
  }
