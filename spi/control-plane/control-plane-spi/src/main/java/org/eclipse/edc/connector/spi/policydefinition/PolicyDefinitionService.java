...
/**
 * The following interface is created for the implementation of the policy
 * definition endpoint.
 */
@ExtensionPoint
public interface PolicyDefinitionService {
   ...
   * @return Policy deleted if the policy is deleted correctly, failure
   *     otherwise
   */

  @NotNull ServiceResult<PolicyDefinition> deleteById(String policyId);
  ...
  @NotNull ServiceResult<PolicyDefinition> create(PolicyDefinition policy);

  /**
   * Updates a policy. If the policy does not yet exist, {@link
   * ServiceResult#notFound(String)} will be returned.
   *
