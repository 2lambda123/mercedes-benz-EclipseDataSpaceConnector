...
public interface TransferProcessService {
   ...
   * <p>
   * The return result status only reflects the successful submission of the
   * command.
   *
   * @param transferProcessId id of the transferProcess
   * @return a result that is successful if the transfer process was found and
   *     is in a state that can be completed
   */
  @NotNull ServiceResult<Void> complete(String transferProcessId);
   ...
   */
  @NotNull ServiceResult<Void> terminate(TerminateTransferCommand command);
   ...
   * @param transferProcessId id of the transferProcess
   * @return a result that is successful if the transfer process was found and
   *     is in a state that can be
   *         deprovisioned
   */
  @NotNull ServiceResult<Void> deprovision(String transferProcessId);

  /**
   * Initiate transfer request for type consumer.
   *
   * @param request for the transfer.
   * @return a result that is successful if the transfer process was initiated
   *     with the created TransferProcess.
   */
  @NotNull
  ServiceResult<TransferProcess> initiateTransfer(TransferRequest request);

  /**
   * Asynchronously informs the system that the {@link DeprovisionedResource}
   ...
   */
  ServiceResult<Void> completeDeprovision(String transferProcessId,
                                          DeprovisionedResource resource);

  /**
   * Asynchronously handles a {@link ProvisionResponse} received from an
   * external system
   *
   * @param transferProcessId The transfer process id
   ...
   * @return a result that is successful if the transfer process was found
   */
  ServiceResult<Void> addProvisionedResource(String transferProcessId,
