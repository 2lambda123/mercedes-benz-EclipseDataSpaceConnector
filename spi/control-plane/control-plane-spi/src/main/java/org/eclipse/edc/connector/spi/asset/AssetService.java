...
public interface AssetService {
   ...
   * @return successful result if the asset is created correctly, failure
   *     otherwise
   */
   ...
   * @return successful result if the asset is deleted correctly, failure
   *     otherwise
   */
  ServiceResult<Asset> delete(String assetId);

  /**
   * Updates an asset. If the asset does not yet exist, {@link
   * ServiceResult#notFound(String)} will be returned.
   *
   * @param asset The content of the Asset. Note that {@link Asset#getId()} will
   *     be ignored, rather the separately supplied ID is used
