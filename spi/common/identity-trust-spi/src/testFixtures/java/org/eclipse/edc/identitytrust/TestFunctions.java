...
public class TestFunctions {
  ...
  public static final Issuer TRUSTED_ISSUER =
      new Issuer("http://test.issuer", Map.of());
  ...
  public static VerifiablePresentationContainer createPresentationContainer() {
    return new VerifiablePresentationContainer(
        "RAW_VP", CredentialFormat.JSON_LD,
        createPresentationBuilder().type("VerifiableCredential").build());
  }

  public static VerifiableCredentialContainer createCredentialContainer() {
    return new VerifiableCredentialContainer(
        "RAW_VC", CredentialFormat.JSON_LD,
        createCredentialBuilder().type("VerifiableCredential").build());
  }
  ...
  public static TokenRepresentation createJwt(String issuer, String subject) {
    ...
    var claimsSet =
        new JWTClaimsSet.Builder()
            .subject(subject)
            .issuer(issuer)
            .claim(PRESENTATION_ACCESS_TOKEN_CLAIM,
                   createJwt(new JWTClaimsSet.Builder()
                                 .claim("scope", "fooscope")
                                 .build())
                       .getToken())
  ...
  public static TokenRepresentation createJwt(JWTClaimsSet claimsSet) {
    ...
    ECKey ecJwk;
    try {
      ecJwk = new ECKeyGenerator(Curve.P_256).keyID("123").generate();
